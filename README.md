...
**And by the sound, and by the pixels.** A level meter stands beside the preview:
what is being heard while something plays, and the frame under the playhead while
nothing does, so the silence at a junction can be walked up to a frame at a time.
`Z` opens a magnifier showing part of the picture at the recording's own pixels --
the interlace comb that a preview scaled to the window takes out with it.
...
| Platform | File | Notes |
|---|---|---|
| **Linux** | `SmartCut_0.8.3_amd64.AppImage` | Make it executable and run it |
| **Linux** | `SmartCut-0.8.3-linux-x86_64.tar.gz` | Unpack and run `./smartcut`. Use this if you would rather not deal with FUSE |
| **Linux (Debian/Ubuntu)** | `smartcut_0.8.3_amd64.deb` | `sudo apt install ./smartcut_0.8.3_amd64.deb`. Only 4.8 MB, because it uses the FFmpeg already on your system |
| **Windows** | `SmartCut_0.8.3_x64-setup.exe` | Installer. The command-line tool, `smartcut-cli.exe`, goes into the same folder |
| **Windows** | `smartcut-portable-x64-0.8.3.zip` | Unzip and run `smartcut.exe`. The command-line tool is `smartcut-cli.exe` |
...
`tests/` holds the 30 end-to-end suites and 559 checks both sides are run
