...
いったん絵に戻し、また一から作り直します。時間がかかるうえに、全部のフレームが
少しずつ元より悪くなります。
...
**一晩ぶんをディスクとして出せます。** ファイルだけではありません。レコーダーが
...
放送局が文字ではなくドットで送ってくる字、たとえば行の続きを示す矢印も、その絵の
まま描きます。継ぎ目が台詞の途中に来ていないかは、映像だけを見ても分かりません。
...
| プラットフォーム | ファイル | 備考 |
|---|---|---|
| **Linux** | `SmartCut_0.6.7_amd64.AppImage` | 実行権限を付けて起動します |
| **Linux** | `SmartCut-0.6.7-linux-x86_64.tar.gz` | 展開して `./smartcut` を実行します。FUSE を使いたくない場合はこちらです |
| **Linux (Debian/Ubuntu)** | `smartcut_0.6.7_amd64.deb` | `sudo apt install ./smartcut_0.6.7_amd64.deb`。システムに入っている FFmpeg を使うので 3.9 MB で済みます |
| **Windows** | `SmartCut_0.6.7_x64-setup.exe` | インストーラ |
| **Windows** | `smartcut-portable-x64-0.6.7.zip` | 展開して `smartcut.exe` を実行します |
...
1 枚で完結する絵だけです。継ぎ足す断片にはそれで足りますし、実装もフォーマットの
ごく一部で済みます。元のフレームよりビットは食いますが、断片は 1 秒に満たないので
...
運びます。多重化では通り抜けられないので、パケットを録画から取り出して出来上がった
ファイルへバイトのまま配り直します。受信機は同じページを組み立てます。要らなければ
`--no-data-broadcast` で捨てられます。カルーセルは多重の 1/100 から 1/5 を占めるので、
サイズを優先するときはそちらです。

**ディスクの字幕（`.ts` / `.m2ts` で出力する場合）:** Blu-ray は字幕を書くのでは
なく描きます。運ばれるのは 1 枚の絵で、切り離せない複数のパケットに分かれています。
...
**カットの中か、傍らか。** DVD も同じように字幕を描きますが、TS にその置き場所は
...
残す範囲の両端は整えます。`.mp4` に切ってディスクの字幕を残せるのは、傍らに置く
この道だけです。
