...
| **Linux** | `SmartCut-0.4.0-linux-x86_64.tar.gz` | 展開して `./smartcut` を実行します。FUSE を避けたい場合はこちらです |
...
1. **録画を追加します。** ウィンドウにドラッグ＆ドロップするか、**＋ ファイル追加**
   から選びます。行は落とした瞬間に埋まり、読み込み・サムネイル・CM 検出は裏で
   同時に進みます。読み込みの終わっていない録画も、待たずにそのまま開けます。
...
VC-1——2010 年頃までにプレスされた Blu-ray の多くがこれで書かれています——だけは
事情が違います。VC-1 のエンコーダはどこにも存在しないからです。FFmpeg にも、
グラフィックスカードにもありません。そこで SmartCut は範囲の両端の部分 GOP 用に
自前のエンコーダを持っています。書くのはイントラピクチャだけで、繋ぎ込む断片には
それで足り、フォーマットのごく一部で済みます。置き換える元のピクチャよりビットは
食いますが、断片は 1 秒に満たないので割に合います。残りは従来どおりバイト単位の
コピーです。この断片をどれだけ細かく書くかは `--vc1-quant` で 3（最も細かい）から
31 まで指定でき、既定値は元のピクチャに対しておよそ 46dB になります。
...
| VC-1 の Blu-ray（1920x1080i アニメ）、GOP 途中〜GOP 途中 10 秒 | 308/308 フレーム、映像の 90% がバイト一致、書き直した分は 48dB |
| VC-1 の Blu-ray（1920x1080p 実写・強いグレイン）、同 10 秒 | 246/246 フレーム、90% がバイト一致、書き直した分は 45dB でグレインも残存 |
