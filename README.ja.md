...
ビット単位でコピー、再エンコードは 0.57 秒。6743 フレーム中、触ったのは
17 フレームだけ。**
...
- **スマートレンダリング** — H.264 / HEVC / MPEG-2 / MPEG-4 Part 2。
  ...
  **MPEG-2 AAC のまま
  ...
  自分で組み立てて防いでいる。
- **必要なら 5.1ch を畳む**（`--audio-channels 2`、または出力設定画面）。
  サラウンドを受け付けないところで再生しなければならない録画は、音声をコピー
  しないほうがいい唯一の場面である。libav 自身のダウンミックス係数で焼き直し、
  ADTS ヘッダも 2ch に書き換える——トランスポートストリームがチャンネル数を
  名乗るのはそこだからである。
- **字幕と番組情報を持ったまま出る**（TS 出力）。放送が送っている
  ARIB STD-B24 の字幕ストリームを、映像と同じだけずらして持ち越す——
  1 文が 1 パケットに収まるので継ぎ目で焼き直すものが無く、**バイト単位で
  そのまま**である。番組情報（EIT p/f）・放送局名（SDT）・放送時刻（TOT）も
  引き継ぐ: 多重化器はストリームの説明しか書かないので、書き終えた TS を
  1 パスなめて**録画自身の表に置き換える**。各ストリームは**元の PID に
  戻る**ので、録画を前提にした道具から見て素材と同じ形で開く。
  文字スーパーとデータ放送は載せられない——前者はパケットに時刻がなく、
  後者はストリームではなく断片の繰り返しだからで、黙って落とさず
  「持ち出せません」と言う。
- **音声多重放送**を読み、**主音声も副音声も書き出す**。二か国語放送は
  音声を 2 本別々の PID で送っていて、以前は 1 本しか見ていなかった。
  スマート結合は各トラックに独立にかかる——2 本はフレームの位置も継ぎ目の
  ずれも違うので、片方の答えをもう片方に流用できない。要らないほうは
  カット編集画面の**トラック**メニューで外せる。
...
- **クリップ一覧とその裏のバッチ** — 一晩ぶんの録画を入力画面に放り込むと、
  ...
  読んでいないクリップもそのまま開ける。一覧はまるごと**プロジェクト**として
  保存でき（`Ctrl+S`）、録画・カット・出力設定がそのまま次の晩に戻ってくる。
  カット編集は一覧から
...
| `--audio-channels N` | 出力するチャンネル数（1〜8）。素材と違う数はダウンミックス——サラウンドを持て余す再生環境のために 5.1ch を 2ch へ畳む——で、コピーで通せるフレームが無いので `--audio-mode` が何であれ全編を焼き直す |
| `--audio-bitrate RATE` | 焼き直す音声のビットレート。`192k` でも `192000` でも可。省くと素材に合わせ、畳んだときはチャンネル数のぶん下げる |
...
![SmartCut のクリップ一覧](docs/images/list.ja.png)
...
この画面の右上にある **SmartCut** ボタンから環境設定を開ける。表示言語は
ここで選ぶ——日本語・英語・OS の設定に従う（既定）。変更は両方のウィンドウに
その場で反映され、次回起動時も引き継がれる。

同じメニューの **SmartCut について** はバージョン情報を出す——本体とカット
エンジンの版、実際に読み込まれている FFmpeg ライブラリの版とライセンス、
実行環境。ここだけは選択してコピーできるようにしてある。不具合を報せるときは
そのまま貼れる。

同じメニューから**プロジェクト**の保存と読み込みができる（`Ctrl+S` /
`Ctrl+O`）。書き出されるのは一覧そのもの——録画のパス、入れたカットと印、
出力設定。あとから割り出せるものは入れない——シーク用インデックスも CM 検出の
結果もプログラム自身のキャッシュ置き場にあるので、開き直したときに読み直せば
済む。
おかげで `.scproj` は数百バイトで、別のマシンでも、キャッシュを消したあとでも
開ける。`.scproj` はウィンドウに落としても開くし、
`smartcut 金曜の分.scproj` でも開く。

タイトルバーには開いているプロジェクトの名前が出て、ディスクに書いていない
作業があるあいだは頭に `*` が付く。その状態でウィンドウを閉じようとすると
確認が出る。保存済みかどうかは旗を立てるのではなく**突き合わせて**いるので、
編集画面をキャンセルで抜けたり、足したクリップを消したりすれば `*` は
ひとりでに消える。

## 編集画面

![SmartCut のカット編集画面](docs/images/editor.ja.png)
