...
**放送録画の中身がそのまま残ります。** 字幕、番組情報、放送局名、青いボタンの先の
...
| プラットフォーム | ファイル | 備考 |
|---|---|---|
| **Linux** | `SmartCut_0.6.5_amd64.AppImage` | 実行権限を付けて起動します |
| **Linux** | `SmartCut-0.6.5-linux-x86_64.tar.gz` | 展開して `./smartcut` を実行します。FUSE を使いたくない場合はこちらです |
| **Linux (Debian/Ubuntu)** | `smartcut_0.6.5_amd64.deb` | `sudo apt install ./smartcut_0.6.5_amd64.deb`。システムに入っている FFmpeg を使うので 3.9 MB で済みます |
| **Windows** | `SmartCut_0.6.5_x64-setup.exe` | インストーラ |
| **Windows** | `smartcut-portable-x64-0.6.5.zip` | 展開して `smartcut.exe` を実行します |
...
各ストリームは元の位置に置き直します。**データ放送**（青いボタンの向こう側）も
...
ディスクにも使え、どちらも字幕をカットの傍らに置きます。プレーヤーも字幕ツールも
