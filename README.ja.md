...
「前のフレームから変わったところ」だけを持つフレームとでできています。キーフレームの
...
**切る場所を字幕で確かめられます。** 編集画面のプレビューに字幕を出せます（既定は表
示しない）。放送の ARIB 字幕は、放送局が指定した位置と色のまま文字として出します。デ
ィスクの字幕（PGS・DVD のサブピクチャ）は、描かれた絵のまま重ねます。放送局が文字で
はなくドットで送ってくる字、たとえば行の続きを示す矢印も、その絵のまま描きます。継ぎ
目が台詞の途中に来ていないかは、映像だけを見ても分かりません。
...
| プラットフォーム | ファイル | 備考 |
|---|---|---|
| **Linux** | `SmartCut_0.5.17_amd64.AppImage` | 実行権限を付けて起動します |
| **Linux** | `SmartCut-0.5.17-linux-x86_64.tar.gz` | 展開して `./smartcut` を実行します。FUSE を使いたくない場合はこちらです |
| **Linux (Debian/Ubuntu)** | `smartcut_0.5.17_amd64.deb` | `sudo apt install ./smartcut_0.5.17_amd64.deb`。システムに入っている FFmpeg を使うので 3.4 MB で済みます |
| **Windows** | `SmartCut_0.5.17_x64-setup.exe` | インストーラ |
| **Windows** | `smartcut-portable-x64-0.5.17.zip` | 展開して `smartcut.exe` を実行します |
...
ウィンドウにドロップすると、ディスクのどれを読み、そのどのトラックを取り込むかを
選ぶ画面が出ます。この確認は省けません。市販の 1 期ボックスには、ロゴ・警告・
...
DVD は規格上、1 本のつながった映像を 1GB 前後の断片に分けて収めています。SmartCut
は展開もコピーもせずに断片をつなぎ、必要な範囲だけを取り出します。書き出しは
MPEG-TS です。DVD の形に戻すには、SmartCut が書かない表がいくつも要ります。
...
**4K HDR10** も切れます。カットの境目で書き直すフレームにも、録画が持っている HDR の設定
をそのまま持たせるので、途中で見え方が変わることはありません。4K 放送が HLG を書くと
きの後方互換の書き方、つまりストリームの 2 か所がわざと別のことを言う書き方も、その
まま再現します。**Dolby Vision** はコピーする限り丸ごと残ります。書き直すフレームには持
たせられないので、書き直しが出るカットではそう表示します。
...
これをまとまりごと引き継ぎ、残す範囲の両端を繕います。範囲が開いた時点ですでに
出ていた字幕は最初のフレームで出し直し、範囲が終わる時点で出ている字幕は下ろし
ます。次の場面まで出しっぱなしにはしません。
[ディスクが描く字幕](docs/technical/disc.ja.md#ディスクが描く字幕)を参照してください。

**カットの中か、傍らか。** DVD も同じように字幕を描きますが、TS にその置き場所はあり
ません。そこで既定では**変換します**。Blu-ray が運ぶ種類として、同じ画素と同じ色で書
き直すので、カットは字幕の入った 1 ファイルになります。残る 2 つはどちらのディスクに
も使え、どちらも字幕をカットの傍らに置きます。プレイヤーも字幕ツールもそのまま読める
 **`.idx` と `.sub`**（VobSub）か、ディスプレイセットそのものである**`.sup`** です。
`.sup` は、Blu-ray の字幕ならバイト単位でそのままです。どちらでも残す範囲の両端は繕
います。`.mp4` に切ってディスクの字幕を残せるのは、傍らに置くこの道だけです。
...
| VC-1 の Blu-ray（1920x1080i アニメ）、キーフレームの間から間まで 10 秒 | 308/308 フレーム、映像の 90% がバイト一致、残りは 48dB で書き直し |
...
エンジンは Rust のクレート 1 つです。GUI とコマンドラインはその前に立つ 2 つの
...
オラクルが同意しないエンジンの変更は、正しいとは認めません。Python が今も残って
いる理由はそれだけです。
