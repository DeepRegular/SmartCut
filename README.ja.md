...
| プラットフォーム | ファイル | 備考 |
|---|---|---|
| **Linux** | `SmartCut_0.5.5_amd64.AppImage` | 実行権限を付けて起動します |
| **Linux** | `SmartCut-0.5.5-linux-x86_64.tar.gz` | 展開して `./smartcut` を実行します。FUSE を使いたくない場合はこちらです |
| **Linux (Debian/Ubuntu)** | `smartcut_0.5.5_amd64.deb` | `sudo apt install ./smartcut_0.5.5_amd64.deb`。システムの FFmpeg にリンクするので 3.2 MB で済みます |
| **Windows** | `SmartCut_0.5.5_x64-setup.exe` | インストーラ |
| **Windows** | `smartcut-portable-x64-0.5.5.zip` | 展開して `smartcut.exe` を実行します |
