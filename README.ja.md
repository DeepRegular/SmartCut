...
| プラットフォーム | ファイル | 備考 |
|---|---|---|
| **Linux** | `SmartCut_0.5.2_amd64.AppImage` | 実行権限を付けて起動します |
| **Linux** | `SmartCut-0.5.2-linux-x86_64.tar.gz` | 展開して `./smartcut` を実行します。FUSE を使いたくない場合はこちらです |
| **Linux (Debian/Ubuntu)** | `smartcut_0.5.2_amd64.deb` | `sudo apt install ./smartcut_0.5.2_amd64.deb`。システムの FFmpeg にリンクするので 3.2 MB で済みます |
| **Windows** | `SmartCut_0.5.2_x64-setup.exe` | インストーラ |
| **Windows** | `smartcut-portable-x64-0.5.2.zip` | 展開して `smartcut.exe` を実行します |
...
持たせるので、途中でトーンマッピングが変わることはありません。VP9 と AV1 は
非対応です。連結できるエレメンタリストリーム形式を持たないため、対応するには
別の設計が必要になります。
