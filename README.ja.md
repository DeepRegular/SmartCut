...
| **Linux (Debian/Ubuntu)** | `smartcut_0.5.7_amd64.deb` | `sudo apt install ./smartcut_0.5.7_amd64.deb`。システムに入っている FFmpeg を使うので 3.3 MB で済みます |
...
**音声:** AAC と Blu-ray のリニア PCM はスマートレンダリングの対象です。ファイル
内の全トラックを個別にカットするので、二か国語放送は両方の言語が残ります。
...
AC-3・E-AC-3・MP2 はスマートレンダリングではなくコピーで通し、その旨を画面に
表示します。ディスクのロスレス音声（DTS-HD と TrueHD）はバイト単位でそのまま
運び、再エンコードしません。
...
**DVD の字幕:** 同じ絵ですが、TS には置き場所がありません。そこで選べます。
**そのまま傍らに**——プレイヤーがそのまま読める `.idx` と `.sub` の 2 ファイル——か、
**変換して中に**——TS が運べる種類の字幕として書き、カットを 1 ファイルにまとめる。
どちらでも色はディスクの索引にあるパレットを使い、残す範囲の両端も繕います。
[DVD の字幕は傍らに置くか、変換して中に入れる](docs/technical/disc.ja.md#dvd-の字幕は傍らに置くか変換して中に入れる)
を参照してください。
