...
コピーする**スマートレンダリング対応のカットツール。放送録画（MPEG-2 TS）から
CM を落とす用途を主眼に置いている。
...
- **CM 境界の自動検出** — 無音の並びと局ロゴの有無を併用し、15 秒の格子に
  乗る区間を CM ブロックとして出す。境界はアクセスポイントへ吸着させるので、
  **CM カットがまるごと無劣化**になる。
...
| | |
|---|---|
| Linux | `smartcut_0.1.0_amd64.AppImage` — glibc 2.39 以上（Ubuntu 24.04 / Debian 13 / Fedora 40 以降） |
| Windows | `smartcut_0.1.0_x64-setup.exe`（インストーラ）または `smartcut-portable-x64.zip`（解凍して実行）。x64 のみ、WebView2 ランタイムが要る |
...
| [GUI](docs/gui.md) | 編集画面、サムネイル軌道、シーン検出、再生 |
| [CM 境界の検出](docs/cm-detection.md) | 無音とロゴ、15 秒格子、誤検出を出さない設計 |
