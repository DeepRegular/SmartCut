...
# -> target/release/bundle/appimage/SmartCut_0.5.6_amd64.AppImage
```
...
```bash
./SmartCut_0.5.6_amd64.AppImage --appimage-extract >/dev/null
...
```bash
./gui/build-linux.sh
# -> gui/src-tauri/target/release/bundle/linux/SmartCut-0.5.6-linux-x86_64.tar.gz
# -> gui/src-tauri/target/release/bundle/linux/smartcut_0.5.6_amd64.deb
```
...
に従うので `SmartCut_0.5.6_amd64.deb` のようになる。deb のパッケージ名 `smartcut` と
...
# -> gui/src-tauri/target/x86_64-pc-windows-msvc/release/bundle/nsis/SmartCut_0.5.6_x64-setup.exe
