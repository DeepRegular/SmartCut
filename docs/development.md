...
`run_audio_tests.sh` and `run_downmix_tests.sh` build their own fixtures into
the same directory -- an impulse train and a 5.1 track with a tone per channel,
...
and
`run_ts_layout_tests.sh` and `run_broadcast_tests.sh`. They look in `~/media`
by default, which
