...
- [ユーザーガイド](#ユーザーガイド)は使い方の説明です。専門知識は必要ありません。
- [技術資料](#技術資料)は中で何をしているかの解説です。仕組みを知りたい人と、
  コードを触る人のために書いてあります。

各ページに日本語版と英語版があり、冒頭のリンクで切り替えられます。
...
| [CM 検出](user-guide/cm-detection.ja.md) | CM の切れ目を自動で探す機能。どのくらい当たるか、外れたときはどうするか |
| [まとめて処理する](user-guide/batch.ja.md) | 一晩分の録画をまとめて片付ける方法。裏で動く処理、複製、まとめて書き出し、誰もいないあいだにプロジェクトを順に書き出すバッチ出力ツール |
...
「GOP 単位で切って繋ぐだけ」では済まない理由を 11 個挙げてあります。

### 仕組みと検証

| | |
|---|---|
| [アルゴリズム](technical/algorithm.ja.md) | カットを head / body / tail に切り分ける原理と、見た目より難しくしている 11 個の難所 |
...
| [ディスクを書く](technical/bdav.ja.md) | 一晩分のカットを BDAV フォルダーとして書き出す。到着時刻、エントリーポイントマップ、番組名の出どころ、意味が分からないまま実物から写した欄 |
| [ディスク容量に合わせる](technical/transrate.ja.md) | 1 枚に収まらない録画を、デコードせずに映像だけ小さく書き直して収める。再量子化、係数の切り捨て、レート制御、70 万フレームで確かめた同一性 |

### 実装

| | |
|---|---|
| [Rust コア](technical/rust-core.ja.md) | タイムスタンプの生成、SPS/PPS 混在の解決、libavcodec に無いため自作した VC-1 エンコーダ、Rust 実装が Python 実装を追い越した点 |
