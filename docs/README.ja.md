...
[実装上の難所](technical/algorithm.ja.md#実装上の難所)がおすすめです。
「GOP 単位で切って繋ぐだけ」では済まない理由を 8 つ挙げてあります。

## ユーザーガイド

| | |
|---|---|
| [GUI](user-guide/gui.ja.md) | 全画面の操作手順（スクリーンショット付き）。録画の追加からカット、出力設定、書き出しまで |
| [CM 検出](user-guide/cm-detection.ja.md) | CM の切れ目をどうやって見つけているか、精度はどのくらいか、外れたときはどうするか |
| [プロジェクト](user-guide/projects.ja.md) | 一晩ぶんの作業を `.scproj` に保存し、あとから再開する |
| [バッチ処理](user-guide/batch.ja.md) | 一晩ぶんの録画をまとめて処理する。クリップ一覧、裏で動く 3 つの処理、書き出し |

## 技術解説

| | |
|---|---|
| [アルゴリズム](technical/algorithm.ja.md) | カットを head / body / tail に切り分ける原理と、これを見た目より難しくしている 8 つの落とし穴 |
...
| | |
|---|---|
| [Rust コア](developers/rust-core.ja.md) | タイムスタンプの生成、SPS/PPS 混在の解決、libavcodec に無いので自作した VC-1 エンコーダ、Rust 実装が Python 実装を追い越した点 |
