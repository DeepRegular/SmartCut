...
- **映像トラックは 1 本のみ**。音声 1 本のみという制限は Python 参照実装のもので、
  Rust エンジンは録画が持つ音声トラックを全部読んで全部書き、`.ts` に書き出すときは
  ARIB 字幕もそのまま引き継ぐ（[Rust コア](rust-core.ja.md#音声多重放送1-本ではなく全部)
  と[放送ワークフロー互換](broadcast-ts.ja.md)）。PGS など他の字幕形式は未対応。
