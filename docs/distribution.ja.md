...
従うので `SmartCut_0.3.1_amd64.deb` などになる——deb のパッケージ名 `smartcut` と
違うのはそのため。`build-linux.sh` は両方を `tauri.conf.json` から読む。

| 成果物 | サイズ | FFmpeg | 要るもの |
|---|---|---|---|
| `SmartCut-0.3.1-linux-x86_64.tar.gz` | 208.2MB | 同梱 | glibc 2.39 以上。FUSE は不要 |
| `smartcut_0.3.1_amd64.deb` | 2.7MB | システムのものを使う | FFmpeg 7.1（Debian 13 / Ubuntu 25.04 以降） |
...
| 成果物 | サイズ | 中身 |
|---|---|---|
| NSIS インストーラ | 52.9MB | 展開後 168.9MB（exe 10.6MB + FFmpeg DLL 8 個） |
| 可搬 zip | 66.1MB | 同じ一式。解凍して `smartcut.exe` を叩くだけ |
