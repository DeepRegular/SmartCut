...
- **HEVC**: leading picture は RADL と RASL であり、同じエントリポイントの trailing
  picture がそれらを参照することは規格が禁じている。だから符号化器が何をしていようと
  常に捨ててよい。RASL に至っては捨てなければならない。参照先が切り口の向こう側に
  あるからだ。これが `bitstream::leading_always_droppable` である。おかげで、
  計測していない次の点まで待たずに、すぐ次のエントリポイントからコピーを始められる。
  必要になったのは 4K 放送だった。エントリポイントは全部 CRA で IDR が 1 枚も無く、
  ピクチャごとに測ると全部が開始点に使えない判定になる。1 分切り出すと、直す前は
  冒頭 13.3 秒を再エンコードしていた。直したあとは 0.44 秒である（区間の 22.8% が
  1.4% になった）。
...
間隔より小さくなる。29.97 fps のストリームで、絵が 60.05996 秒にあるところへ
...
本来の参照付きで復号した同じ絵と比べて 51 dB。良い再エンコードではあるが、録画
...
### 11. 1 フレームが 2 枚の絵で届くことがある

出力タイムラインはフィールドで数えている。プルダウンは 3 フィールド表示する絵を
含み、フレーム格子では表せないからだ。これは「その絵が何フィールド立つか」の答え
である。「1 フレームが何枚の絵で届くか」の答えではない。そして後者は常に 1 とは
限らない。H.264 は 1 フレームをフィールド 2 枚として符号化できる。PAFF である。
レコーダーが自分で焼く Blu-ray がまさにこれで、1440x1080 29.97 の全フレームが
2 枚に割れている。

放送の 1080i はインターレース素材をフレームとして符号化している。だから放送の
コーパスではここに当たらない。レコーダー盤では 2 箇所が同時に壊れる。

- **配置。** 1 枚は半フレームで、タイムラインを 1 フィールド占める。2 ではない。
  2 を与えると区間の長さは実際の倍になり、各パケットに書く duration も倍になる。
- **先読み。** DTS は表示順が確定するまで絵を抱えておく。何枚抱えるかは録画の
  並べ替え深度から取るが、これは**フレーム**を数えた値である。1 枚しか抱えないと、
  P フレームの復号時刻を、その手前に表示される B フレームが届く前に確定してしまう。
  届いた B は行き場を失って落ちる。40 秒の切り出しで 1842 枚中 642 枚。コピー区間は
  最初の 1 枚からブロックノイズの塊になった。

そこでキューは枚数ではなく**半フレーム単位**で測ることにした。1 フィールド分の絵は
半分を 1 つ、それ以外は 2 つ。3 フィールド表示の絵も 1 フレームなので 2 つ数える。
境界を動かさないためである。これでフレーム符号化の素材は以前とまったく同じだけ
抱えることになり、大半がフレーム符号化で数組だけフィールド対がある録画も、その
数組の周りだけ余裕が増える。

**判定はパケットに対して行う。中の絵に対してではない。** libavcodec の MPEG-2
パーサはフィールド対を 1 パケットに結合する。手元の放送 1 本は 30 秒の間に 7 回
それをやっていた。2 枚の絵、1 フレーム、2 フィールドである。H.264 パーサは
フィールドを 1 枚ずつ渡す。だから数えるのはパケットが**開始する**絵の数にした。
H.264 なら `first_mb_in_slice` が 0 のスライス、MPEG-2 なら picture coding
extension の個数である。開始する絵が 1 枚で、それがフィールドのときだけ半フレーム
と答える。

このフラグを読むにはシーケンスヘッダが先に要る。`field_pic_flag` は `frame_num`
の後ろにあり、その幅は SPS にしか書かれていない。VC-1 のヘッダと同じく録画ごとに
一度だけ読む。`frame_mbs_only_flag` が立っているシーケンスなら、絵を 1 枚も見ずに
済む。ただし**ヘッダだけでは決まらない**。同じ録画に対して本プログラムが書き戻す
のは MB-AFF で、そのヘッダもフィールドを許しているが、中の絵はすべてフレームである。

対の 2 枚目は、自分のタイムスタンプではなく**1 枚目の隣**に置く。2 枚は復号順で
必ず隣り合うので、1 枚目は直前に見た絵で確定する。対に 1 つしかタイムスタンプを
与えない録画、あるいは丸めると同じ位置に来る 2 つを与える録画では、そうしないと
2 枚が同じ場所に重なり、片方が行き場を失う。
