...
# -> target/release/bundle/appimage/SmartCut_0.6.9_amd64.AppImage
```
...
```bash
./SmartCut_0.6.9_amd64.AppImage --appimage-extract >/dev/null
...
```bash
./gui/build-linux.sh
# -> gui/src-tauri/target/release/bundle/linux/SmartCut-0.6.9-linux-x86_64.tar.gz
# -> gui/src-tauri/target/release/bundle/linux/smartcut_0.6.9_amd64.deb
```
...
に従うので、`SmartCut_0.6.9_amd64.deb` になる。deb のパッケージ名 `smartcut` と
食い違うのはこのためである。`build-linux.sh` は両方を `tauri.conf.json` から読む。

| 成果物 | サイズ | FFmpeg | 必要条件 |
|---|---|---|---|
| `SmartCut-0.6.9-linux-x86_64.tar.gz` | 209.9 MB | 同梱 | glibc 2.39 以上。FUSE 不要 |
| `smartcut_0.6.9_amd64.deb` | 4.0 MB | システムのものを使用 | FFmpeg 7.1（Debian 13 / Ubuntu 25.04 以降） |
...
# -> gui/src-tauri/target/x86_64-pc-windows-msvc/release/bundle/nsis/SmartCut_0.6.9_x64-setup.exe
