...
`productName` に従うので、`SmartCut_0.8.9_amd64.deb` になる。deb のパッケージ名
`smartcut` と食い違うのはこのためである。`build-linux.sh` は両方を
`tauri.conf.json` から読む。

| 成果物 | サイズ | FFmpeg | 必要条件 |
|---|---|---|---|
| `SmartCut-0.8.9-linux-x86_64.tar.gz` | 202.4 MB | 同梱 | glibc 2.39 以上。FUSE 不要 |
| `smartcut_0.8.9_amd64.deb` | 4.8 MB | システムのものを使用 | FFmpeg 7.1（Debian 13 / Ubuntu 25.04 以降） |
