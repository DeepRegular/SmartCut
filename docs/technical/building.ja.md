...
bash tests/run_udf_tests.sh           # イメージをファイルシステムとして実物と並べる  50
...
フォルダーとファイル単位・バイト単位で一致することである。レコーダーで編集を続ける
ためのイメージ（`--iso-access overwritable`）も同じように確かめる。加えて、見かけの
大きさが BD-RE 1 枚分あり、実際に使う容量は中身の分だけであることも確かめる。
...
揃っていて 50 件、実物が 1 枚も無ければ 45 件になる。並べて出た違いは失敗ではない。
