...
`run_proxy_tests.sh`）は、フィクスチャが無いと、黙って検査の半分を飛ばすのではなく
...
確かめる。最後がイメージで、UDF の版ごとに書き、このプログラムで開き直し、7-Zip が
入っていれば別人の書いた読み手で展開して元のフォルダーと突き合わせる。
...
そしてトランスポートストリームなら全フレームの ADTS ヘッダである。また、コーデックが
持たないレートを指定されたときは、持っているうちで最も近いものに直したうえで、その旨
を表示しなければならない。量子化ビット数はサンプルをそのまま書くコーデックでしか
意味を持たないので、LPCM では指定に従い（そこではファイルの大きさそのものが決まる）、
ほかのコーデックでは理由を示して断る。
...
でその `.m2ts`（あるいはその一部）を指定する。指定が無い場合は、成功したふりをせず
にそこで終了する。さらに、通常の `cargo build --release` では作られないエンコーダ側の
example バイナリも必要である。
