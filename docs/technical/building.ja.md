...
あること、何かの上に何かが書かれていないこと、エクステントが長さの欄を超えないこと、
ストリームが 64 KB クラスタの頭から始まること、そして木構造が元のフォルダーと
ファイル単位・バイト単位で一致すること。
...
（量子化ビット数）である。リサンプルは 3 か所に反映されなければならない。サンプル自体、
...
タイムライン・インターレース・音声と映像の長さの差をまとめて検査する。結果は
