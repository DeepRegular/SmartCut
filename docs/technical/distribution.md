...
# -> target/release/bundle/appimage/SmartCut_0.8.5_amd64.AppImage
```
...
```bash
./SmartCut_0.8.5_amd64.AppImage --appimage-extract >/dev/null
...
```bash
./gui/build-linux.sh
# -> gui/src-tauri/target/release/bundle/linux/SmartCut-0.8.5-linux-x86_64.tar.gz
# -> gui/src-tauri/target/release/bundle/linux/smartcut_0.8.5_amd64.deb
```
...
`SmartCut_0.8.5_amd64.deb` and the like — which is why they and the deb's package name
`smartcut` differ. `build-linux.sh` reads both out of `tauri.conf.json`.

| Artifact | Size | FFmpeg | Requires |
|---|---|---|---|
| `SmartCut-0.8.5-linux-x86_64.tar.gz` | 202.4 MB | Bundled | glibc 2.39 or newer. No FUSE needed |
| `smartcut_0.8.5_amd64.deb` | 4.8 MB | Uses the system's | FFmpeg 7.1 (Debian 13 / Ubuntu 25.04 and later) |
...
# -> gui/src-tauri/target/x86_64-pc-windows-msvc/release/bundle/nsis/SmartCut_0.8.5_x64-setup.exe
