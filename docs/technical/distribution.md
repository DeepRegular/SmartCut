...
`SmartCut_0.5.9_amd64.deb` and the like — which is why they and the deb's package name
`smartcut` differ. `build-linux.sh` reads both out of `tauri.conf.json`.

| Artifact | Size | FFmpeg | Requires |
|---|---|---|---|
| `SmartCut-0.5.9-linux-x86_64.tar.gz` | 209.4 MB | Bundled | glibc 2.39 or newer. No FUSE needed |
| `smartcut_0.5.9_amd64.deb` | 3.4 MB | Uses the system's | FFmpeg 7.1 (Debian 13 / Ubuntu 25.04 and later) |
