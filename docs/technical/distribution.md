...
`SmartCut_0.5.16_amd64.deb` and the like — which is why they and the deb's package name
`smartcut` differ. `build-linux.sh` reads both out of `tauri.conf.json`.

| Artifact | Size | FFmpeg | Requires |
|---|---|---|---|
| `SmartCut-0.5.16-linux-x86_64.tar.gz` | 209.5 MB | Bundled | glibc 2.39 or newer. No FUSE needed |
| `smartcut_0.5.16_amd64.deb` | 3.4 MB | Uses the system's | FFmpeg 7.1 (Debian 13 / Ubuntu 25.04 and later) |
...
to care about FUSE. Being gzip, it is 24 MB larger than the squashfs+zstd AppImage.
...
|---|---|---|
| NSIS installer | 53.5 MB | 171.1 MB installed (12.7 MB exe plus 8 FFmpeg DLLs) |
| Portable zip | 66.8 MB | The same set. Unzip and run `smartcut.exe` |
