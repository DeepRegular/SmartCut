...
`SmartCut_0.6.5_amd64.deb` and the like — which is why they and the deb's package name
`smartcut` differ. `build-linux.sh` reads both out of `tauri.conf.json`.

| Artifact | Size | FFmpeg | Requires |
|---|---|---|---|
| `SmartCut-0.6.5-linux-x86_64.tar.gz` | 209.7 MB | Bundled | glibc 2.39 or newer. No FUSE needed |
| `smartcut_0.6.5_amd64.deb` | 3.9 MB | Uses the system's | FFmpeg 7.1 (Debian 13 / Ubuntu 25.04 and later) |
