...
The artifact is **186.0 MB and carries all 716 shared libraries**. WebKitGTK 4.1 is in
...
`SmartCut_0.8.0_amd64.deb` and the like — which is why they and the deb's package name
`smartcut` differ. `build-linux.sh` reads both out of `tauri.conf.json`.

| Artifact | Size | FFmpeg | Requires |
|---|---|---|---|
| `SmartCut-0.8.0-linux-x86_64.tar.gz` | 202.0 MB | Bundled | glibc 2.39 or newer. No FUSE needed |
| `smartcut_0.8.0_amd64.deb` | 4.5 MB | Uses the system's | FFmpeg 7.1 (Debian 13 / Ubuntu 25.04 and later) |
