...
- **ヒステリシスは濃さだけでなく時間にも要る。** 消えたほうは 1 サンプルで
  信じてよいが、**戻ったほうは平滑化の窓のぶん続かないと不在を終わらせない。**
  長い CM の途中で、たまたまテンプレートに似た絵の CM がしきい値を 1〜2
  フレームだけかすめることがある。ここで区間が割れると見た目以上に悪い:
  断片がそれぞれブロックになり、端はそれぞれ最寄りの継ぎ目へ吸着するので、
  **前のブロックの終わりが次のブロックの始まりを追い越す**。BS アニマックスで
  302 秒の CM が 3 つに割れ、うち 2 つが 1.6 秒重なっていた。
...
全フレーム復号の 8 分の 1）。GUI は常に併用する——選ばせるには当たり外れの
事情が細かすぎるし、外したときに落ちる精度のほうが 30 秒より高くつく。
CLI では `--logo` で明示的に併用する。

**字幕リセットが見つかった録画では、ロゴは読まない。** そちらが強いうえに
10 倍速いので、併用の指定があっても素通りする。
...
「CM を検出」は、CM ブロックの頭と本編への戻りを**キーフレームとして**
並べるだけで、カットするのは人間。「CM を除いた区間にする」ボタンは一度
作って外した——どのみち境界を目で確かめたいうちは、一発でやる道は使われない
（[GUI](gui.ja.md#カット編集画面別ウィンドウ)）。

その目印で、両端を**アクセスポイントに吸着**させて（「無劣化点へ吸着」、
もしくはもともとそこに落ちている境界）カットすれば、CM カットは丸ごと
コピーになる。30 分の民放録画を処理すると本編 22.6 分が 5 区間として残り、
**無劣化コピー 100.0%**。単一区間の例では
