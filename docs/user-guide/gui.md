...
| `Black{` and so on | Top left | The frame a stretch begins on, the brace on the side the stretch lies. A stretch one frame long is `{Black}` |
| `}Black` and so on | Top right, before `]` | The frame a stretch ends on |
...
**Sound only, no pictures**, at the foot of the container list, turns the
output into an audio file. The ranges that were cut, the order the clips were joined in and the
...
| **Silence is quieter than** | -50 dB out of the box, measured sample by sample. Lower finds only what is truly silent. A stretch begins and ends on the sample the sound stopped or came back on, rather than on the frame that sample is in |
