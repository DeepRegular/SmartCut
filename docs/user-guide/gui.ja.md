...
| **音声だけを出力する（映像なし）** | カットも結合も継ぎ目のフェードも入ったまま、音声だけのファイルを書き出します。映像は読みも書きもしません。→ [音声だけを出力する](#音声だけを出力する) |
...
**音声だけを出力する（映像なし）** をオンにすると、出力は音声ファイルになります。
...
どれにも当てはまらないものは `.m4a` になります。選ぶものが無いので、
コンテナタイプの欄は灰色になります。
