...
また再生します。継ぎ目に playhead を置いて `Space` を押せば、その継ぎ目だけが何度も
再生されます。**始めた位置に戻る**ので、IN の手前から聞きたいときは IN の手前で始めて
ください。選択範囲を決めていない録画では、範囲は録画ぜんぶです。

**音量**は**プレビューだけ**の音量です。書き出す音は変わりません。♪ を押すと音が消え、
もう一度押すと戻ります（`M` も同じです）。スライダの上でホイールを回しても動きます。
音量とミュートは覚えているので、次にクリップを開いたときも同じ大きさで鳴ります。
音声のない録画では、♪ もスライダも押せません。
...
| 設定 | 内容 |
|---|---|
| **PageUp / PageDown** | そのキーの動きです。`Shift`、`Ctrl`、`Shift+Ctrl` の 4 通りに、それぞれ数と単位を持たせられます。単位は**フレーム移動**・**秒移動**・**% でスクロール**の 3 つです。前の 2 つは 1 回押すごとの移動量です。% は移動速度で、いちばん速いスクロール（60 倍速）に対する割合です。25 なら 15 倍速で、1 秒押しっぱなしにすると録画の 15 秒ぶん進みます。録画の長さには左右されません。既定は 15 フレーム、`Shift` で 30 フレーム、`Ctrl` で 25 %、`Shift+Ctrl` で 50 % です。0 にすると、そのキーは動きません |
| **両方あるときに読むのは** | 録画の隣に `.keyframe` と `.trim.avs` の両方があるときに、どちらを読み込むかです |
| **ショートカットでの保存は、確認せずに上書きする** | `Ctrl+H` と `Ctrl+Shift+H` が、同じ名前のファイルを黙って上書きします |
