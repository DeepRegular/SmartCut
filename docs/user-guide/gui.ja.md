...
字幕、メニュー——が並びます。映像は外せません。**字幕はカットに付いていきます。**
...
- 放送の**文字字幕（ARIB）**は、放送局が指定した位置・大きさ・色のまま文字として
  ...
  ——次の行へ文を続ける矢印、話者名を囲む二重括弧、ドイツ語の `ü` ——も、その点から
  文字と同じ枠に描きます。
...
| **ディスクの字幕** | ディスクから取り込んだ録画のときだけ出ます。既定は `カットの中に (PGS)`——字幕の入った 1 ファイルになります。傍らに置くこともできます。`.idx / .sub` はプレイヤーも字幕ツールもそのまま読める 2 ファイル、`.sup` はディスプレイセットそのものです。**どれが無加工かは行に書いてあります**。DVD と Blu-ray とで別のものが無加工になるからです |
