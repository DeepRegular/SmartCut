...
| 中央 | プレビュー。映像の下端中央にフレーム番号・時刻・そのフレームの種類・選択範囲が出ます。下端中央の表示は最下段の**カウンタ**で消せます。左上と右上には選択範囲やキーフレームの印が出ます（下の表） |
...
プレビューの左上と右上に出る印は次のとおりです。いま表示しているフレームに
当てはまるものだけが出ます。
...
| `黒{` `}黒` など | 左上 | 区間の始まりのフレームは `黒{`、終わりのフレームは `}黒` のように、区間のある側に括弧が付きます。1 フレームだけの区間は `{黒}` です |

黒白と無音の札は、検出した区間そのものから出します。区間の端をキーフレームに
したかどうかは関係ありません。区間の終わりの位置は、環境設定の
「黒白区間の終わりの印」に従います。

印の背景は半透明なので、下の局ロゴや時刻の表示も透けて見えます。
要らないときは、環境設定の「カット編集で、選択範囲・キーフレーム・黒白無音の印を
映像の上に表示する」を外してください（既定はオン）。カウンタとは別の設定です。
...
| **カット編集で、選択範囲・キーフレーム・黒白無音の印を映像の上に表示する** | カット編集の映像の左上と右上に出る印です。既定はオンです |
