...
`Ctrl+H` と `Ctrl+Shift+H` は、画面を出さずに、録画と同じ名前でそのまま保存します。
同じ名前のファイルがあれば確認します。確認せずに上書きする設定もあります。
CM 検出結果にショートカットはありません。
...
複数あるときにどれを読むかは、環境設定で決めます。1 つだけなら、設定に関わらず
それを読みます。

**このファイルを読んだときは、CM 検出結果の印を混ぜません。**`.keyframe` は自分で
...
| **複数あるときに読むのは** | 録画と同じ名前の `.keyframe`、`.trim.avs`、`.cm.json` が複数あるときに、どれを読むかです |
| **CM を検出したら、自動でキーフレームを置く** | 既定は入です。外すと印は置かず、タイムラインに帯が出るだけになります。見てから置きたいときは ≡ の「CM 検出結果をキーフレームにする」です。録画と同じ名前のファイルを読んだときは、入れてあっても印は置きません |
| **ショートカットでの保存は、確認せずに上書きする** | `Ctrl+H` と `Ctrl+Shift+H` が、同じ名前のファイルを確認なしに上書きします |
