...
| **サンプリング周波数** | `入力と同じ` のほか 48 / 44.1 / 32 kHz。素材と違うレートはリサンプルになり、**全編を焼き直します**。指定したレートを扱えないコーデックのときは、扱えるうちで最も近いレートで書きます |
| **量子化ビット数** | `入力と同じ` のほか 16 bit・24 bit。`リニア PCM` のときだけ有効です。ほかのコーデックは音そのものではなく音の記述を書くので、ビット数の置き場所がありません。非圧縮トラックの大きさはこれで決まり（チャンネル数 x ビット数 x サンプリング周波数）、ビットレートの代わりに表示される値がそれです |
| **音声ビットレート** | 焼き直すときのビットレートです。`おまかせ` はコーデックとチャンネル数に見合った値を選びます |
...
**音声コーデック・チャンネル・ビットレートは、モードが `すべて再エンコード` の
ときだけ操作できます。** この 3 つはエンコードの内容を指定するものであり、
ほかの 2 つのモードは全編のエンコードを行わないからです。灰色になっているあいだも
値は保持されるので、モードを戻せばそのまま使えます。
