...
`バッチに登録` は、いま書かずにキューへ回します。一覧はその場でプロジェクトとして
保存されます。保存の画面は出ません。まだ名前の無い一覧は、キュー専用の置き場に、
先頭の行の名前で控えを書きます。**出力先には出しません。** その控えはキューの
持ち物です。バッチ出力ツールでそのジョブを消すと、一緒に消えます。登録すると
