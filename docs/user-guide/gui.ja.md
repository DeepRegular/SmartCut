...
書き出す形式は 2 つあります。保存先には録画のパスが最初から入っています。
...
保存画面で名前の拡張子を `.keyframe` か `.avs` に書き換えれば、そちらが優先されます。
読み込みも同じで、選んだファイルの拡張子が形式を決めます。
...
付くだけです。Trim はカットされた状態で開きます。** 両方あるときにどちらを読むかは
環境設定で決めます。片方しか無いときは、設定に関わらずそれを読みます。
...
| **両方あるときに読むのは** | 録画の隣に `.keyframe` と `.trim.avs` の両方があるときに、どちらを読み込むかです |
