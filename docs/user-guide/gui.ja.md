...
**ディスクは 1 件ではなく、中の録画ぶんの行になります。** `BDAV`・`BDMV`・`VIDEO_TS`
フォルダーを含むフォルダーでも、`.iso` でも、それらが入ったフォルダーでも同じです。
`.iso` はマウントしません。中のストリームをその場で読みます。
...
DVD の行は、ディスク名とタイトル番号です。ディスクが打ったチャプターは最初から
タイムラインに並びます。タイトルが別々に多重化された断片から組まれている場合——
途中でタイムスタンプが振り出しに戻る場合——は、断片ごとの行になり `(1/2)` `(2/2)`
と付きます。2 つの時計を継ぐのは、1 つを切るのとは別の操作だからです。

**VC-1 で書かれたディスクも、ほかと同じようにカットできます。** 2010 年頃までに
プレスされた Blu-ray の多くがこれで、範囲の両端を作り直すための VC-1 エンコーダは
FFmpeg にもグラフィックスカードにも無いので、そのピクチャは SmartCut 自身が書きます。
ウィンドウ側に設定は何もありません。操作は同じで、各範囲の両端の数十枚は、継ぎ目が
見えない程度に細かく書かれます。どれだけ細かく書くかは、コマンドラインの
`--vc1-quant` で指定できます。その代償は
[Rust コア](../developers/rust-core.ja.md#エンコーダのないコーデック-vc-1)にあります。
...
**行は落とした瞬間に埋まります。** 尺・範囲・解像度・フレームレート・コーデック・
音声の有無は、コンテナ自身が名乗っている答えで、ファイルの長さに関係なく 30 ミリ秒で
返ります。左の絵も同じ時点で、近似シークで取った 1 枚が入ります。20 本落としても、
最後の行がパスと空の四角のまま数分待たされることはありません。

そのうえで、裏では **3 本の列**が同時に進みます。1 本はパケットを走査して
シーク用インデックスを作り、1 本はキーピクチャを復号してサムネイルとシーンの
変わり目を作り、1 本は CM を検出します。索引はディスクに残るので、同じ録画の
2 回目からは作りません。

![読み込みの最中の一覧](../images/usage-loading.ja.png)

行の右下に進み具合が出ます。`読み込み中` は走査、`サムネイル` は絵と
シーンを作る復号のパスで、まだ順番の来ていない行は `待機中` です。走査が終わると
`索引 2 秒`（今回作った）か `前回の索引を再利用`（以前の結果がそのまま使えた）に
変わります。ウィンドウの右上には、どの列がいまどのクリップを見ているかが出ます。
上の図では 2 本目を走査しながら、1 本目のサムネイルを作っているところで、
3・4 本目はまだ順番待ちです。それでも 4 行とも、素性と絵はもう出ています。

走査の答えが出ると、行の数字はそちらで置き換わります。2 つが食い違うのは尺で、
DVD のプログラムストリームは自分の長さを記録していないため、コンテナの答えが
外れることがあります。

**どの列も編集画面を待たせません。** まだ読み込みの終わっていないクリップも
...
使い方です。検出は 1 本ずつ順に、索引作りやサムネイル作りと並行して進みます。

**検出は索引を待ちません。** 手掛かりを探す 3 つの読み取り——字幕・音声・ロゴ——は
録画を頭から読むだけで済みます。索引が要るのは、見つけた境界を実際のシーンの
変わり目へ載せる最後の仕上げだけなので、それはパスの終わりに行われます。読み取りは
分単位、走査は秒単位なので、その番が来る頃には走査はとうに終わっています。
...
走査を待たずに入れたカットは、入れた瞬間の時刻にそのまま立ちます。あとから
無劣化点が届いても動きません。変わるのは**その代償**の答えだけで、継ぎ目が
アクセスポイントに乗っていなければ数フレームの再エンコードとして計画に現れ、
...
**音声をどう書くかの項目は、書くときにだけ現れます。** 下の表のうち 5 つ——コーデック・
チャンネル・サンプリング周波数・量子化ビット数・ビットレート——はどれもエンコードの
話で、ほかの 2 つのモードは全編のエンコードを行いません。どのモードでも灰色のまま
5 行並べておく代わりに、`すべて再エンコード` が求めたときにだけ現れます。
...
上の出力フォーマットの音声行は 1 クリップの説明なので、常にそのクリップ自身の
値が出ます。
...
| **編集画面の絵が粗い / 出るのが遅い** | まだ読み込みの途中です。1 段階目のプレビューは近似シークの絵で、走査が終わるとコマ精度になり、サムネイルができるとフィルムストリップも埋まります（[開いた瞬間から使えます](#開いた瞬間から使えます)）。索引は初回だけ作ります |
