...
| **結合** | `リストを1つのファイルにまとめる`。オフなら 1 行につき 1 ファイルで、これまでどおりです。オンにすると、リストの並び順のまま 1 本のファイルとして書き出します。名前は先頭の行から取ります。→ [リストを 1 つのファイルにまとめる](#リストを-1-つのファイルにまとめる) |
| **基準クリップ** | チェックを入れているあいだだけ、その隣に出ます。解像度・フレームレート・コーデック・音声トラックを、どの行に合わせるかを選びます |
...
**リストを1つのファイルにまとめる** にチェックを入れると、1 行 1 ファイルでは
...
まとめる設定にすると、出力設定画面の上に **継ぎ目の効果** が現れます。

![継ぎ目の効果の設定](../images/usage-cross.ja.png)

| 項目 | |
|---|---|
| **対象クリップ** | どの継ぎ目の設定かを選びます。効果は手前のクリップに属するので、最後の行以外が並びます |
...
| **すべての継ぎ目に適用** | 上の設定をリスト内の全部の継ぎ目に入れます。12 話に同じ効果を 12 回指定しなくて済みます |
...
音声はミックスしません。手前のクリップの音が効果のあいだも鳴り続け、次の
クリップの音は効果の終わりから始まります。こうすると前後どちらも自分の映像と
ずれません。切り替わりが唐突に感じられるときは、[環境設定](#出力設定)の
継ぎ目のフェードで音量を落として戻せます。
