...
| **バッチ出力** | 一覧ウィンドウ（タブ 4 枚目） | 保存したプロジェクトをキューに並べる |
| **バッチ出力ツール** | **別ウィンドウ・別プロセス** | キューを上から順に書き出す。本体を閉じても止まらない。[まとめて処理する](batch.ja.md#一晩かけてプロジェクトを順に処理する) |

カット編集が別ウィンドウなのは、上の 3 つが「一覧全体について一度決める設定」
なのに対し、カットは 1 本ずつやる作業で、「この 1 本は終わった」という区切りが
要るからです。その区切りが **OK** ボタンです。

バッチ出力は作業の段階ではありません。一覧 1 つではなく、いくつもの一覧に
ついての画面なので、最後に置いてあります。キューを実際に書き出すのは別ウィンドウ
のツールで、これも別プロセスです。深夜に並べたキューは、並べた窓を閉じたあとも
書かれ続けなければならないからです。
...
されてから（名前が無ければここで訊かれます）キューの末尾に入り、あとで
[バッチ出力ツール](batch.ja.md#一晩かけてプロジェクトを順に処理する)がまとめて
書き出します。
