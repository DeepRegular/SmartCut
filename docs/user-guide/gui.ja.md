...
| **ディスクの字幕** | ディスクから取り込んだ録画のときだけ出ます。既定は `カットの中に (PGS)` で、字幕の入った 1 ファイルになります。傍らにも置けます。`.idx / .sub` はプレーヤーも字幕ツールもそのまま読める 2 ファイル、`.sup` はディスプレイセットそのものです。**どれが無加工かは行に書いてあります**。DVD と Blu-ray とで別のものが無加工になるからです |
...
| **番組名** | **クリップごと**の設定です。ディスクの索引に、その録画の名前として入ります。録画が自分について持っている情報で埋めておくので、必要なら上から書き換えてください。空にすると、また元の名前に戻ります |
...
付いたこと、ディスクの索引やイメージが書けなかったこと、イメージができたので
...
**データ放送も残す（.ts のみ）** は、青いボタンの先にあるページをカットにも
残します。入るのは放送自身のテーブルを持つ `.ts` だけなので、ディスクや `.mp4`
...
| 設定 | 内容 |
|---|---|
| **範囲の先頭を整え直す** | 開いた GOP から始まる範囲で、先頭の最大 2 秒を再エンコードします。継ぎ目の乱れは減りますが、無劣化でコピーされる区間はその分減ります。CLI の `--clean-joins` と同じ設定です |
| **プロキシを作ってから編集する** | 録画全体を再エンコードし、軽い映像で編集します。1 時間あたり数分の時間と数 GB の容量が必要です。1 フレームの展開が重い素材では効果的です。幅も選べます |
...
変更後に作成されたものだけが新しい場所に保存されます。既存のものはそのまま残ります。
