...
右の「黒白・無音を検出」で、選んだ録画すべてを検出します。映像が真っ黒・
真っ白な区間と、音声が無音の区間の 2 つを探します。CM 検出とは別の処理で、
どちらか一方だけを動かせます。

全フレームをデコードするので、CM 検出より時間がかかります。進み具合は行に
出ます（`黒白・無音 検出中 33% — 映像を調べています`）。終わると
`黒白・無音: 黒白 1 箇所 / 無音 4 箇所` のように残ります。
...
再生中は絵が止まります。フレームごとに読み直すと再生の邪魔になるためで、
...
上の「黒白を検出」「無音を検出」で、いま開いている録画を調べます。一覧で
検出済みの録画なら、開いた時点で結果が並んでいます。押すと読み直します。
...
| **黒・白の区間とみなす長さ** | これ以上続いた区間だけを扱います。単位はフレームと秒から選べます。既定は 2 フレームです。放送では 2〜4 フレームの黒が多いためです |
| **無音の区間とみなす長さ** | 同じく無音のほうです。既定は 0.4 秒です。CM の切れ目の無音は 1 秒前後、会話の間は 0.1〜0.4 秒なので、その境目にあたります |
...
| `Alt+↑` `Alt+↓` | 前の / 次の検出結果の端へ（黒・白と無音の区間） |
