...
隣に複数あるときにどれを読むかは、環境設定で決めます。1 つだけなら、設定に関わらず
それを読みます。

**隣のファイルを読んだときは、CM 検出結果の印を混ぜません。**`.keyframe` は自分で
...
| **複数あるときに読むのは** | 録画の隣に `.keyframe`、`.trim.avs`、`.cm.json` が複数あるときに、どれを読むかです |
| **CM を検出したら、自動でキーフレームを置く** | 既定は入です。外すと印は置かず、タイムラインに帯が出るだけになります。見てから置きたいときは ≡ の「CM 検出結果をキーフレームにする」です。録画の隣のファイルを読んだときは、入れてあっても印は置きません |
