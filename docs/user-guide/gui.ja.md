...
- データ放送はこのメニューで切り替えるトラックではありません。置けるのは `.ts`
  だけなので、出力設定画面で一覧全体にまとめて答えます。そこで外していれば、
...
| **ファイル名の接頭辞** | 既定は `cut_` で、ファイル名の先頭に付きます。既定値は[環境設定](#出力設定-1)で変えられます |
...
| **データ放送も残す (.ts のみ・ファイルは大きくなる)** | 録画のデータ放送（青いボタンの向こう側）をカットにも残します。**データ放送を積んだ録画が一覧にあり、`.ts` で書くときだけ**出る項目で、そこでは**オン**です。カットは録画を短くしたものだからです。サイズを優先する回だけ外してください。局によってファイルの 1/100 から 1/5 を占めます |
