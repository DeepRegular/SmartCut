...
深夜に並べたキューが、並べた窓を閉じたあとも書かれ続けなければならないからです。
...
- フレーム番号と時刻の表示は、字幕を出していても絵の**下**のままです。字幕と同じ場所
  ...
  フィルムストリップの下の行が言い続けますし、外したことは次のクリップでも覚えています。
- 再生中も追従します。
...
CM の先頭のキーフレームを押して `I`。
続いて本編に戻る位置のキーフレームを押し、`←` で 1 フレーム戻ってから `O`。
...
**CM を落とすだけなら完全無劣化になることが多い**です。この 21 フレームは、
...
| **ディスクの字幕** | ディスクから取り込んだ録画のときだけ出ます。既定は `カットの中に (PGS)` で、字幕の入った 1 ファイルになります。傍らにも置けます。`.idx / .sub` はプレイヤーも字幕ツールもそのまま読める 2 ファイル、`.sup` はディスプレイセットそのものです。**どれが無加工かは行に書いてあります**。DVD と Blu-ray とで別のものが無加工になるからです |
...
| **ディスクの種別** | イメージの下にあり、イメージを作るときだけ出ます。そのイメージを焼いたディスクに何をしてよいか、を書き分けます。既定の「追記しない」は、BD-R や再生するだけの BD-RE について本当のことです。「レコーダーで編集する」はレコーダーが BD-RE に書く値で、ディスクに録画を足したり消したりする前にレコーダーが見たがるのはこちらです。焼いたディスクをレコーダーに戻すならこちらを選んでください |
| **イメージを作成後、フォルダーを削除** | イメージの下にあり、イメージを作るときだけ出ます。同じ 30GB が 2 つ残るのは、たいていの用には多すぎます。フォルダーは今までどおり書かれ、`.iso` もそれを元に作られます。削除はそのあとで、`.iso` が書けたときだけです。消えるのは `BDAV` です。その上のフォルダーは空になったときだけ消えるので、自分のフォルダーに直接書いた場合、そこにある他のものは残ります |
...
住んでいます。番組名、録ったチャンネル、記録日時、放送局が書いた番組内容です。
...
切れ目に加えて、カット編集で打った印も入ります。ディスクではこれが
