...
多いためです。カット編集を開かずに書き出した場合も、残した範囲にある
チャプターは出力に引き継がれます。
...
**長さは、カットしたあとの長さです。** カットが入っている行では、録画そのものの
長さは「カット前」として同じ行に残ります。画面の上の合計時間も同じで、カットが
1 つでも入っていれば、書き出されるぶんの合計と、括弧に入れた録画そのものの合計の
両方が出ます。録画そのものの長さは、下のクイックプロパティにも出ています。
...
| `Delete` / `Backspace` | 一覧から外す（**ファイル自体は消えません**） |
| `Ctrl+Z` / **削除を元に戻す** | 外した行を戻す。カットも印も検出結果も、外す前のまま元の位置に戻ります。「全削除」も戻せます。さかのぼれるのは直近 20 回までで、プロジェクトを開いたり新規作成したりすると、それ以前の分は戻せなくなります。外したあとで同じファイルを追加し直した場合、その行は戻しません |
...
その下の 2 行も同じです。コーデックや検出結果が途中で切れているときは、
その行にマウスを乗せると全部が出ます。

右端の印の 1 つが**編集済**です。カット編集で何かを行った行に付きます。
解析や検出だけを行った行には付きません。検出は SmartCut の答えであって
誰かが決めたことではないので、印の数や CM の数を見ても、その録画を自分で
見終えたのかどうかは分からないためです。20 本の一覧で、済んだものと
まだのものを見分けるための印です。キャンセルで閉じた場合は付きません。
この印はプロジェクトにも保存されるので、翌日に開き直しても残っています。
...
複数選んでいるときは、本数と「音声:」の行だけを表示します。

クイックプロパティの「音声:」の行は、コーデックとサンプリング周波数に続けて、
その行の音声を**何で書き出すか**を下線付きで示します（例: `aac, 48000 Hz, ステレオ (初期値)`）。
下線の部分を押すと選択肢が開きます。これは**その行だけ**の設定です。
音声トラックが 2 本以上ある録画では「音声 1:」「音声 2:」のように 1 本ずつ行が
分かれ、トラックごとに選べます。

- 先頭の「(初期値)」は、出力設定画面の音声チャンネルに従ったときの結果です。
  出力設定が 2ch で録画が 5.1ch なら「ステレオ (初期値)」、出力設定が
  「入力と同じ」なら録画そのままの「5.1ch (初期値)」になります。
- その下に、録画から畳める形（7.1ch・5.1ch・ステレオ・モノラル）が並びます。録画より
  多いチャンネル数は出ません。モノラルの録画のように選べる形が 1 つしかないときは、
  グレーのまま変えられません。
- 番組情報でデュアルモノラル（二か国語を 1 本のトラックに左右で入れたもの）と
  分かる音声は「デュアルモノラル」と表示し、畳む選択肢は出しません。1 チャンネルに
  畳むと 2 つの言語が混ざってしまうためです。

**選んでいる行すべてに適用されます**ので、12 話ぶんを全選択してから 1 回選べば
済みます。トラックが複数ある行を選んでいるときは、1 本目どうし、2 本目どうしで
1 行ずつ出ます。行によって答えが違うときは「選んだ行で設定が違います」と出ます。
そこで選び直せば全部そろいます。

この項目だけが行ごとの設定です。出力設定画面のほかの項目は、出力されるファイルに
ついての設定なので一覧全体で 1 つの答えしかありません。チャンネル数は違います。
5.1ch の映画と 2ch の放送が 1 つの一覧に並んでいるとき、映画だけをステレオに
畳みたい、というのは普通の作業だからです。

カット編集の**プレビュー音声と音声レベルメーターも、ここで選んだ形で鳴り・振れます**。
5.1ch の録画をステレオにした行なら、編集画面ではステレオに畳んだ音が聞こえ、
メーターも 2 本になります。編集画面を開いたまま選び直してもかまいません。再生中なら
1 秒ほどで切り替わります。出力設定の音声チャンネルで畳む場合も同じです。

(初期値) 以外を選ぶと、**そのトラックは再エンコードになります**。チャンネル数を
変える方法がそれしかないためです（コピーやスマートレンダリングは録画のフレームを
そのまま運びます）。選択肢の横にも「このトラックは再エンコードされます」と出ます。
出力設定の音声が「そのままコピー」のときは選べません。

結合して 1 ファイルにするときは、**基準クリップの答え**が使われます。音声
トラックの宣言はファイルに 1 つしかないためです。
...
予約した検出は、行の右クリックメニューで取り消せます。
複数の行を選んでおけば、まとめて取り消せます。

| 検出 | メニューの項目 |
|---|---|
| CM 検出 | 「CM 検出予約を取り消す」 |
| 黒白検出 | 「黒 検出予約を取り消す」（名前は環境設定で探すものに合わせて変わります） |
| 無音検出 | 「無音 検出予約を取り消す」 |

以前に検出したことのある行は、前回の結果に戻ります。すでに始まっている検出は
取り消せません。止めるには「解析を中止」を押してください。
...
`無音なし` です。黒白検出のバッジの名前は、探すものに合わせて変わります。
既定の「黒だけ」なら `黒 検出予約` `黒 検出中` `黒 6` `黒なし` です。
一度も検出していない録画には、その検出のバッジは出ません。
...
途中で止めたいときは「解析を中止」を押します。読み込み中でも CM 検出の途中でも、
すぐに止まります。止まるまでのあいだ、ボタンは「中止しています…」になります。

止まったあと、まだ解析していない録画が残っていれば、ボタンは「解析を再開」に
変わります。押すと残りの録画から再開します。途中で止めた録画は、その録画の
最初からやり直します。
...
| 左の列 | **キーフレーム**（印）の一覧。サムネイル付きで、押せばそこへ飛びます。黒白検出・無音検出が置いた印には `黒` `白` `無音` の札が付きます。下のほうの印を押しても、スクロール位置はそのままです |
| 中央 | プレビュー。映像の下端中央にフレーム番号・時刻・そのフレームの種類・選択範囲が出ます。左上と右上には選択範囲やキーフレームのインジケータが出ます（下の表）。下端の表示は最下段の**カウンタ**で、インジケータは**インジケータ**で消せます |
| プレビューの左 | **音声レベルメーター**。再生中は鳴っている音、止めているときは再生位置の音の大きさです（環境設定で消せます） |
| その下の帯 | **フィルムストリップ**。前後のフレームをサムネイルで並べたものです。1 画面に映す秒数は右の「1 画面」で選べます。既定の「自動」では、キーフレームの間隔に合わせて、1 GOP がサムネイル 1 枚分の幅になるように秒数を決めます（2〜60 秒）。セルの幅は、そのセルが表す時間に比例します。いまの位置は中央の縦線で示します。「1 フレームずつ」のときは 1 セルが 1 フレームなので、縦線の代わりに、表示中のフレームを青い枠で囲みます |
| シークバー | 緑の部分が出力になる範囲。`▼` がキーフレーム、くすんだ赤の縦線がカットの継ぎ目、下の細い目盛りが場面の変わり目。その下の 2 段が、黒・白の区間と無音の区間です。**再生位置は `◎` と、上から下まで通った明るい赤の縦線**です。`◎` は緑の帯の中にしかないので、その上下に並ぶ印や区間と再生位置が合っているかどうかは、この線で読みます |
| ボタンの列 | 中央が**カット**、その左が `[ IN`、右が `OUT ]`。外へ向かって「移動」「1 フレーム」「無劣化点」「先頭・末尾」と対称に広がります |
| 下の帯と行 | **書き出しの計画**。どこをコピーして、どこを作り直すか |
| 最下段 | 左にカットのコスト、その隣に**カウンタ**・**インジケータ**・**字幕**（プレビューに何を出すか）。右に **OK** と**キャンセル** |

プレビューの左上と右上に出るインジケータは次のとおりです。いま表示しているフレームに
当てはまるものだけが出ます。
...
| `]` | 右上 | 選択範囲の末尾（OUT） |
| `⚑` | 左上 | キーフレームを置いたフレーム |
| `黒` `白` `無音` | 左上 | 検出した区間に入っているフレーム |
| `黒{` など | 左上 | 区間の始まりのフレーム。区間のある側に括弧が付きます。1 フレームだけの区間は `{黒}` です |
| `}黒` など | 右上（`]` の左） | 区間の終わりのフレーム |

黒・白・無音のインジケータは、検出した区間そのものから出します。区間の端を
キーフレームにしたかどうかは関係ありません。区間の終わりの位置は、環境設定の
「黒白区間の終わりの印」に従います。

4:3 の録画や、ウィンドウが映像より横に広いときは、映像の左右に黒い余白が
できます。インジケータが収まる幅があれば、映像に重ねずにその余白に出します。
収まらないときは映像の角に重ねます。背景は半透明なので、下の局ロゴや時刻の
表示も透けて見えます。

表示したくないときは、最下段の**インジケータ**を押してください（既定はオン）。
環境設定の「カット編集で、選択範囲・キーフレーム・黒白無音のインジケータを
映像の上に表示する」と同じ設定です。カウンタとは別なので、フレーム番号だけを
消すこともできます。
...
> 2:3 プルダウンの録画について。毎秒 24 フレームで作ったアニメや映画を
> 29.97 fps で放送するときは、ピクチャの一部を 3 フィールドぶん表示させる
> RFF フラグを使い、4 枚のピクチャで 5 フレームを埋めます。SmartCut は
> テレビや DVD プレーヤーと同じように、これを 5 フレームに組み直して表示します。
> 5 フレームのうち 2 枚は前後のピクチャが 1 フィールドずつ混ざったフレームなので、
> 動いている輪郭に縞が出ます。これは録画のとおりの見え方です。
> 書き出しはピクチャ単位のままで、RFF フラグは元の録画のとおりに書きます。
> カットの前後で作り直す部分も同じです。0.8.6 までは、作り直した部分だけ RFF が
> 落ちていて、ピクチャが 2 フィールドぶんしか表示されませんでした。
> 混ざったフレームを IN にすると、書き出しはそこから始まるピクチャからになります
> （表示との差は 1 フィールド、60 分の 1 秒です）。
> 0.8.6 までは 4 枚のピクチャをそのまま並べていたので、5 フレームに 1 枚は
> 対応するピクチャがありませんでした。「1 フレームずつ」のストリップで
> そのセルが黒くなり、1 フレーム送りでも飛ばされていたのはこのためです。
...
| **ホイール** | 1 目盛り 1 フレーム。`Shift` を足すと無劣化点単位で飛びます。編集画面のどこで回しても同じで、拡大表示のウィンドウの上でも効きます。左の印の一覧やプランの上では、そちらのスクロールになります |
...
プレビューを**クリック**すると、その周りを拡大して表示します。押したまま
動かせば、見る場所も一緒に動きます。倍率はウィンドウの下で 2 倍から 8 倍まで
選べます。

拡大表示の中の画像は、**ドラッグで動かせます**。8 倍では画面に入るのが
録画の 200 画素ほどなので、輪郭を端から端までたどるには、プレビューを
何度もクリックするよりこちらが早いはずです。

拡大表示の上でホイールを回すと、カット編集と同じく 1 フレームずつ前後に
送れます。`Shift` を足すと無劣化点単位です。縞を見ながらフレームを
送りたいときに、手をカット編集へ戻さずに済みます。
...
- 字幕のない録画では、選択欄そのものが表示されず、最下段に並ぶのは「カウンタ」
  と「インジケータ」だけです。このページのほかのスクリーンショットがそうなって
  いるのは、練習用の録画に字幕が無いためです。
...
の札が付きます。始まりの印は `黒{`、終わりの印は `}黒` のように、区間のある側に
括弧が付きます。札は区間の端に立っている印にだけ付き、両方に当てはまるときは
2 つ並びます（CM の切れ目は、黒へのフェードと無音が重なっていることがよく
あります）。片方の端の印を消しても、残ったほうの札はそのままです。
...
番号は、いずれも録画の最初のフレームから数えます。カットしたあとで読み込んでも、
印は録画の同じフレームに付きます。`Trim` は AviSynth と同じ読み方をします。
`Trim(300,0)` のように 2 つ目が 0 なら末尾まで、`Trim(300,-10)` のように負の数なら
そのフレーム数だけを残します。
...
- 片方を打ち直しても、もう片方は動きません。範囲を詰めるときは IN と OUT を
  ...
  もう片方は端まで退きます。
- IN と OUT を同じフレームに置くと、そのフレームだけを選びます。
- 選んだ範囲は、プレビューの下と下の行に
...
| **無劣化点へ吸着** | 選んだ範囲の両端を、いちばん近い無劣化点へ寄せます。OUT は範囲の最後のフレームなので、無劣化点の 1 フレーム手前に置きます。本編はその無劣化点から始まります。押せば再エンコードがゼロになります |
...
**IN と OUT を置き直したときも 1 段です。** キー・ボタン・シークバーの
つまみのドラッグ・無劣化点へ吸着、どれも取消で 1 つ前の範囲に戻ります。
CM の頭を探すときは、IN を置いて、少し動かして、もう一度置き直す、という
作業になるので、置く前の範囲へ戻れると探し直しが要りません。つまみの
ドラッグは、動かした量にかかわらず 1 段です。
...
| **基準クリップ** | **クリップを結合して出力** をオンにしたときだけ、そのすぐ下に出ます。解像度・フレームレート・コーデック・音声トラックを、どの行に合わせるかを選びます。形式が違う行があるときは、そのすぐ下に「どの行の何が違うのか」が 1 行ずつ出ます |
...
| **サブフォルダー** | 出力先の下に、この名前のフォルダーを作ってそこへ書きます。2 本以上のときに出ます（結合して 1 ファイルにするときは出ません）。名前は最初に開いたときに入ります。ディスクならディスク名、プロジェクトを保存してあればその名前、どちらでもなければ今日の日付です。空にすれば、上のフォルダーへ直接書きます。同名のフォルダーが既にあれば枝番が付き、`night` の次は `night-2`、`night-3` になります。2 回目の出力が 1 回目のファイルを上書きすることはありません。欄には入力した名前が残り、実際の書き出し先は横の `同名のフォルダーあり → night-2` で分かります |
...
| **コンテナタイプ** | ファイルの形式です。`入力と同じ` か、特定のものを選びます。一覧の録画を収められないコンテナは灰色になります（[灰色になっている選択肢](#灰色になっている選択肢)）。いちばん下の **音声のみ（映像なし）** は、映像を書かずに音声ファイルだけを出力します → [音声だけを出力する](#音声だけを出力する) |
| **音声** | `スマートレンダリング（既定）` / `そのままコピー` / `すべて再エンコード` |
| **音声コーデック** | `入力と同じ` のほか AAC・AC-3・DTS・リニア PCM |
| **音声チャンネル** | `入力と同じ` のほか 1ch・2ch・5.1ch・7.1ch。一覧全体への答えです。行ごとに変えたいときは、入力設定のクイックプロパティの「音声:」の行で選びます。AC-3 と DTS は 5.1ch までしか書けないので、7.1ch の録画では 5.1ch 以下を選ぶまで灰色になります |
...
| **ディスクの字幕** | ディスクから取り込んだ録画のときだけ出ます。既定は `出力ファイルに含める (PGS)` で、字幕の入った 1 ファイルになります。別ファイルにも書き出せます。`.idx / .sub` はプレーヤーも字幕ツールもそのまま読める 2 ファイル、`.sup` はディスプレイセットそのものです。DVD と Blu-ray とで無加工になるものが違うので、どれが無加工かは選択肢の表記に書いてあります。ディスクに書き出すときは、この設定に関係なく出力ファイルに含めます |
...
1 行を選んでそこから全部を取ります。チェックボックスのすぐ下に出る
ドロップダウンで指定します。何も選ばなければ先頭の行です。
...
書き直すのかは、出力設定画面の基準クリップの下に 1 行ずつ出ます。書き出し中にも、
出力画面に次のように出ます。
...
| **イージング** | 進行の加減速です。12 種類の曲線に、イン／アウト／イン-アウト／アウト-イン の 4 モードを組み合わせます。`なし` は等速です |
| **重ねる画像** | 継ぎ目に重ねる静止画です。タイトルやカードに使えます。効果の前 4 分の 1 で浮かび上がり、真ん中の半分は全部出て、後ろ 4 分の 1 で消えます。途中で現れたり消えたりはしません |
| **音声のフェード** | その継ぎ目で音をどう扱うかです。「前のクリップ（フェードアウト）」は手前の音が消えるまでの秒数、「次のクリップ（フェードイン）」は次の音が戻るまでの秒数で、0.1 秒単位・10 秒までです。0 でフェードなしです |
...
切り替わりが唐突に感じられるときは、**音声のフェード**で音量を落として戻せます。

**前と後ろは別々に指定します。** 継ぎ目は 2 つの問いだからです。終わるほうの
番組をどう終わらせるか、始まるほうをどう始めるか。自分のテーマ曲で終わる番組は
長く落としたいけれど、次の番組は頭から普通に始めたい、ということがあります。
前を 3 秒・後ろを 0 秒にすれば、そのとおりになります。1 本の録画の中のカット点に
入れるフェードは [環境設定](#カットの精度と速度)の側で、こちらは継ぎ目ごとの
設定です。

映像の効果とは関係がありません。効果が `なし` の継ぎ目にもフェードは入れられ、
ディゾルブにフェードを入れないこともできます。効果は前後のクリップから素材を
取って成立しますが、フェードは残る素材の上に書くだけなので、長さは変わりません。

**音声をコピーする設定では効きません。** コピーは録画のフレームをそのまま運ぶ
ことなので、書き換えるところがありません。出力設定の「音声」を
スマートレンダリング（既定）か、すべて再エンコードにしてください。
**右のプレビューにも掛かります。** `▶ 再生` を押せば、書き出したファイルと
同じ曲線で音が落ちて戻ります。何秒が適当かは聞いてみないと決められないので、
映像の効果と同じ扱いです。

放送の録画では、音声トラックが映像より少し（0.5 秒ほど）短いことがあります。
この場合、フェードアウトは音の終わりで終わります。映像の継ぎ目まで音が無い
ためです。その隙間は次のクリップの音で埋まります（フェードを入れなくても
同じです）。

### 音声だけを出力する

コンテナタイプで **音声のみ（映像なし）** を選ぶと、出力は音声ファイルになります。
カットした区間も、結合した順番も、継ぎ目のフェードも、映像を書き出したときと
同じように入っています。違うのは、映像が入っていないことだけです。

**映像は読みも書きもしません。** 出力用に映像を書いてから音を取り出すのでは
なく、音声だけの書き出し経路を通ります。11 分の録画 2 本で 1 秒もかかりま
せん（映像つきだと 4 秒ほどです）。

ファイル名の拡張子は、**音声の種類**で決まります。
...
| MP3 | `.mp3` |
| DTS | `.dts` |
| リニア PCM（`音声コーデック` で選んだ場合を含む） | `.wav` |
| そのほか | `.m4a` |

音声の設定はそのまま効きます。既定のスマートレンダリングなら、継ぎ目の
数フレームだけを作り直して残りは録画のフレームそのままです。チャンネル数や
サンプリング周波数を変えれば、その時点でトラック全体の再エンコードになります
（映像つきのときと同じです）。

**音声トラックは 1 本だけです。** 二か国語の録画の 2 本目は入りません。
結合した録画のうち音声の無いものは飛ばし、出力画面にその旨を表示します。

この設定はファイル出力のものです。ディスク出力のタブに切り替えると、コンテナ
タイプの設定に関わらず映像つきで書き出します。ファイル出力に戻れば、選んだ設定は
そのまま残っています。
...
| **ディスクの種別** | イメージの下にあり、イメージを作るときだけ表示されます。そのイメージを焼いたディスクに何をしてよいかを書き分ける項目です。既定の「追記しない（BD-R / 再生専用）」は、BD-R や、再生するだけの BD-RE に当てはまる値です。「レコーダーで編集する（BD-RE）」はレコーダーが BD-RE に書く値で、ディスクに録画を足したり消したりするときにレコーダーが参照します。焼いたディスクをレコーダーに戻すなら、こちらを選んでください。イメージはレコーダーが書くのと同じ配置になり、大きさもディスク全面（25 GB または 50 GB）になります。中身の無い部分は、保存先のディスクで場所を取りません |
...
切れ目に加えて、カット編集で打った印も入ります。ディスクから開いた録画なら、
残した範囲にあるディスクのチャプターも入ります。ディスクでは視聴者が使うのが
...
状況の行は `"…" を仕上げ中: 放送の番組情報を書き戻しています…` に変わります。バーは
2 段階を通して進むので、止まったように見えることはありません。ディスクに
書くときのクリップも同じです。

書き出しの間、このボタンは `出力中止` に変わります。押すと、いま書き出して
いるクリップを最後まで書き終えてから止まります。途中まで書かれたファイルが
残ることはありません。書き出しの間は、プロジェクトを開いたり新規作成したり
できません。

出力するファイルが、一覧にある別の録画と同じファイルになる行は、書き出さずに
エラーにします。出力先が別の録画と同じフォルダーで、接頭辞が空のときなどに
起きます。そのまま書くと、その録画は自分の順番が来る前に上書きされて壊れます。
...
何箇所目かは画像の上の `再エンコード 1 / 2` に、全部で何箇所・何フレームかは
画像の下の「再エンコード:」の行に表示されます。
...
結合して出力するとき、基準クリップと形式が違うクリップも別です。そのクリップは
全フレームが書き直されるので、継ぎ目は出てきません。代表画像と
`基準クリップの形式に合わせて全編を再エンコード` が表示され、下の行には
どこが違うのか（画面サイズ、フレームレート、コーデックなど）が並びます。
どのクリップがこうなるかは、**書き出す前に**出力設定で分かります。基準クリップの
右に本数が出て、そのすぐ下に、どの行のどこが違うのかが 1 行ずつ並びます。

**走査方式の違いでは再エンコードになりません。** インターレースかプログレッシブ
かは、ストリームが書いてある値ではなく、libavformat が冒頭のピクチャから割り出した
読みです。日本の放送は 1080i の中にプログレッシブ符号化のフレームを日常的に入れる
ので、同じ局の同じ番組でも、録画が本編から始まったか直前の CM から始まったかで
答えが変わります。実際、1 本の録画の中で両方のフレームが混ざっています
（ある 2 本を 60 秒ぶん数えると、31 % 対 69 % と 72 % 対 28 % でした）。
ピクチャは 1 枚ずつ自分の種類を持っているので、そのままつないで問題ありません。
...
| **カット編集で、選択範囲・キーフレーム・黒白無音のインジケータを映像の上に表示する** | カット編集の映像の左上と右上に出るインジケータです。既定はオンです。カット編集の「インジケータ」ボタンと同じ設定です |
...
| **番組中の短い挿入（局の ID など）も検出する** | 既定はオフです。CS の一部の局は、地上波で CM が入っていたところに自局の動画 ID を 2〜9 秒だけ差し込みます。ふつうの CM ブロックより短いので、オフのままでは拾いません。見分けているのは音で、挿入は音がいったん切れます。ただし**番組の全画面テロップも同じように隅のロゴを隠します**。テロップの間だけ音を落とす番組では、それも拾います。ある番組で 20 話ぶん測ったところ、出たものの 4 分の 1 ほどがテロップでした。カット編集の「CM を検出」だけが対象で、入力一覧からの検出では拾いません |
...
| **黒とみなす明るさ／白とみなす明るさ／その明るさが占める割合** | 何をもって「黒いフレーム」「白いフレーム」と呼ぶかです。画素の明るさと、そういう画素が画面に占める割合の 2 つで決めます。既定は黒 4 %、白 99 %、割合 98 % です。明るさは、放送の黒を 0 %、白を 100 % とした目盛りで表します。0 % にすると、黒そのものの画素だけを黒と数えます。8 ビットでも 10 ビットでも同じ値が使えます。「明るさの単位」を輝度値にすると、8 ビットの輝度（黒 16、白 235）で直接入力できます。既定の 4 % は 24、99 % は 232 です。黒はその値以下、白はその値以上の画素を数えます。10 ビットの録画にも同じ値で指定でき、中では 4 倍して比べます。フェードの終わりが真っ黒まで落ちない放送では、黒の値を 9〜11 % に上げると拾えるようになります。局のロゴや焼き込みの字幕、時計が出たままの録画では、割合を 95 % ほどまで下げてください。割合を上げると、黒と数えるフレームは減ります。ただし、区間の数が減るとはかぎりません。よくあるのは、黒地にクレジットが出る場面です。文字が出ているあいだは割合が下がるので、長い黒の区間がそこで途切れます。途切れた前後がどちらも長さの条件を満たしていれば、区間の数はかえって増えます。値は区間の数ではなく、見つかった区間の中身を見て決めてください。0.8.5 までは、明るさを 0〜255 全体に対する割合で表していました。放送の黒は 16 なので、6 % 以下では黒そのものが範囲に入らず、何も見つかりませんでした。以前の値は、新しい目盛りに換算して引き継ぎます。画面のいちばん外側 2 % は、どの値でも判定に使いません。放送の映像は端に半端な画素が並ぶことがあるためです。ここを変えると、それまでの検出結果は使えなくなるので、各行の結果はいったん消え、検出し直すと録画を読み直します |
| **黒白区間の終わりの印** | 「黒でなくなったフレーム」（既定）と「最後の黒いフレーム」から選びます。区間の始まりと終わりの 2 つで切ると黒がちょうど無くなるのは前者です。他のツールは「最後の黒いフレーム」を区間の終わりと呼ぶので、数字を突き合わせると 1 フレームずれて見えます。そちらに合わせたいときは後者にしてください。タイムラインの帯もこの設定に従って描きます。無音の区間には影響しません。音の切れ目はサンプル単位で決まり、どちらを選んでも同じ位置になるためです。すでに置いた印は動きません |
...
| **無音とみなす音量** | 既定は -50 dB です。1 サンプルごとに判定します。小さくすると、本当に何も鳴っていないところだけを拾います。区間の始まりと終わりは、音が止まった／戻ったサンプルそのものです |
...
消すのは SmartCut が名前を付けたファイルだけなので、自分のフォルダーをキャッシュの
場所に選んでも、ほかのファイルは残ります。
