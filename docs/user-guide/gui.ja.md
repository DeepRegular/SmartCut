...
予約をやめたいときは、行を右クリックして「CM 検出予約を取り消す」を選びます。
黒白検出と無音検出の予約も、同じメニューの「黒白 検出予約を取り消す」
「無音 検出予約を取り消す」で取り消せます。複数の行を選んでおけば、まとめて
取り消せます。前に検出したことのある行は、その結果に戻ります。すでに検出が
始まっている行は取り消せないので、止めるには「解析を中止」を使ってください。
...
| **黒白区間の終わりの印** | 「黒でなくなったフレーム」（既定）と「最後の黒いフレーム」から選びます。区間の始まりと終わりの 2 つで切ると黒がちょうど無くなるのは前者です。他のツールは「最後の黒いフレーム」を区間の終わりと呼ぶので、数字を突き合わせると 1 フレームずれて見えます。そちらに合わせたいときは後者にしてください。タイムラインの帯も同じ答えで描きます。無音の区間は変わりません。音が戻るのはサンプル単位なので、2 つの答えのあいだにフレームがありません。すでに置いた印は動きません |
