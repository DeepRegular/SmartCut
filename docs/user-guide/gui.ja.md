...
| `Shift+←` `Shift+→` | 1 秒 |
| `PageUp` `PageDown` | 環境設定で決めた量だけ戻る・進む。`Shift`、`Ctrl`、`Shift+Ctrl` にそれぞれ別の量を持たせられます |
| `↑` `↓` | 前の / 次の**無劣化点**へ |
...
| 設定 | 内容 |
|---|---|
| **PageUp / PageDown** | そのキーで動く量です。`Shift`、`Ctrl`、`Shift+Ctrl` の 4 通りに、それぞれ数と単位を持たせられます。単位は**フレーム移動**・**秒移動**・**% でスクロール**の 3 つで、% は録画全体に対する割合です（カットしたあとの長さから数えます）。0 にすると、そのキーは動きません |
...
| **プロキシを作ってから編集する** | 録画全体を再エンコードし、軽い映像で編集します。1 時間あたり数分の時間と数 GB の容量が必要です。1 フレームの展開に時間がかかる素材で効果があります。幅も選べます |
| **継ぎ目の音のフェード** | 継ぎ目の手前で音を下げ、継ぎ目の先で戻します。秒数で指定し、0 でフェードなし（既定）。CLI の `--audio-fade` と同じ設定です |

フェードは音の段差を「間」に変えます。その代わり、指定した秒数ぶんの本編が録画より小さい音に
なります。**音を書き直すときにだけ効きます。** 出力設定の音声がスマートレンダリングか再エンコード
なら掛かり、コピーなら掛かりません。コピーのときは出力のメッセージでそう伝えます。無劣化のまま
運ぶ音声（TrueHD、DTS-HD MA）も同じで、書き直さないので掛かりません。

出力の先頭と末尾には掛かりません。フェードは 2 つの断片が出会う場所のためのもので、
...
シークインデックス・プロキシ・CM 検出の結果を保存する場所です。既定では OS が
...
| `←` `→` | 1 フレーム（押しっぱなしで連続） |
| `Shift+←` `Shift+→` | 1 秒 |
| `PageUp` `PageDown` | 環境設定で決めた量だけ移動（`Shift` `Ctrl` `Shift+Ctrl` で別の量） |
| `↑` `↓` | 前の / 次の無劣化点へ |
| `I` / `O` | ここを選択の開始 / 終わりに |
| `K` | いまのフレームをキーフレームに登録 |
| `S` / `Shift+S` | 次の / 前の場面の変わり目へ |
