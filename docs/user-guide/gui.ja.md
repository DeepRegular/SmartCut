...
| **黒とみなす明るさ／白とみなす明るさ／その明るさが占める割合** | 何をもって「黒いフレーム」「白いフレーム」と呼ぶかです。画素の明るさと、そういう画素が画面に占める割合の 2 つで決めます。既定は黒 10 %、白 92 %、割合 98 % です。明るさは録画の階調いっぱいに対する割合なので、8 ビットでも 10 ビットでも同じ値が使えます。フェードの終わりが真っ黒まで落ちない放送では、黒の値を 14〜16 % に上げると拾えるようになります。局のロゴや焼き込みの字幕、時計が出たままの録画では、割合を 95 % ほどまで下げてください。画面のいちばん外側 2 % は、どの値でも判定に使いません。放送の映像は端に半端な画素が並ぶことがあるためです。ここを変えると、それまでの検出結果は答えにならないので、各行の結果はいったん消え、検出し直すと録画を読み直します |
