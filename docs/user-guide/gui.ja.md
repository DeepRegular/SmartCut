...
| `Shift+←` `Shift+→` | 1 秒 |
| `PageUp` `PageDown` | 環境設定で決めた秒数だけ戻る・進む。`Ctrl` と `Shift` には別の秒数を持たせられます |
| `↑` `↓` | 前の / 次の**無劣化点**へ |
...
画面の下、OK の左に **キーフレーム情報を保存…** があります。いまの編集を録画の隣に
書き出します。**押すと形式を聞きます。** 保存先には録画のパスが最初から入っています。
...
`Ctrl+S` と `Ctrl+Shift+S` は、画面を出さずに録画の隣へそのまま保存します。
...
| 設定 | 内容 |
|---|---|
| **PageUp / PageDown** | そのキーで動く秒数です。`Ctrl` と `Shift` には別の秒数を持たせられます。0 にすると、そのキーは動きません |
| **両方あるときに読むのは** | 録画の隣に `.keyframe` と `.trim.avs` の両方があるときに、どちらを読み込むかです |
| **ショートカットでの保存は、確認せずに上書きする** | `Ctrl+S` と `Ctrl+Shift+S` が、同じ名前のファイルを黙って上書きします |
...
| `←` `→` | 1 フレーム（押しっぱなしで連続） |
| `Shift+←` `Shift+→` | 1 秒 |
| `PageUp` `PageDown` | 環境設定で決めた秒数だけ移動（`Ctrl` `Shift` で別の秒数） |
| `↑` `↓` | 前の / 次の無劣化点へ |
| `I` / `O` | ここを選択の開始 / 終わりに |
| `K` | いまのフレームをキーフレームに登録 |
| `S` / `Shift+S` | 次の / 前の場面の変わり目へ |
| `Ctrl+D` | CM を検出 |
| `Ctrl+S` | キーフレーム情報を録画の隣に保存 |
| `Ctrl+Shift+S` | Trim を録画の隣に保存 |
| `Esc` | 捨てて閉じる（キャンセルと同じ） |
