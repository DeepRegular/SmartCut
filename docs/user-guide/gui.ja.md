...
その下の **出力の音声チャンネル** は、**その行だけ**の設定です。既定は
「出力設定に従う」で、出力設定画面の音声チャンネルがそのまま使われます。
1ch・2ch・5.1ch を選ぶと、その行だけその本数で書き出されます。**選んでいる行
すべてに適用されます**ので、12 話ぶんを全選択してから 1 回選べば済みます。
行によって答えが違うときは空欄になり、そこで選び直せば全部そろいます。
...
本数を選ぶと、**その行の音声は再エンコードになります**。本数を変える方法が
それしかないためです（コピーやスマートレンダリングは録画のフレームをそのまま
運びます）。出力設定の音声が「そのままコピー」のときは選べません。録画より多い
本数も選べません。
...
| **音声チャンネル** | `入力と同じ` のほか 1ch・2ch・5.1ch。一覧全体への答えです。行ごとに変えたいときは、入力設定のクイックプロパティで設定します |
