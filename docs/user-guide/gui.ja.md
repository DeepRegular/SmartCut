...
済みます。複数選んでいるときも「音声:」の行だけは表示され、行によって答えが違うときは
「選んだ行で設定が違います」と出ます。そこで選び直せば全部そろいます。
...
(初期値) 以外を選ぶと、**その行の音声は再エンコードになります**。チャンネル数を
変える方法がそれしかないためです（コピーやスマートレンダリングは録画のフレームを
そのまま運びます）。行の横にもその旨が出ます。出力設定の音声が「そのままコピー」の
ときは選べません。
...
入れるフェードは [環境設定](#カット編集)の側で、こちらは継ぎ目ごとの設定です。
...
ファイル名の拡張子は、**音声の種類**で決まります。放送の録画なら `.aac`、
AC-3 なら `.ac3`、`音声コーデック` で リニア PCM を選べば `.wav` です。
どれにも当てはまらないものは `.m4a` になります。コンテナタイプの下の行に、
いまの一覧ならどれになるかが出ます。
