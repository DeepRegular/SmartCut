...
右の「黒白を検出」（`Ctrl+B`）と「無音を検出」（`Ctrl+Q`）で、選んだ録画
すべてを検出します。映像が真っ黒・真っ白な区間を探すのが前者、音声が無音の
区間を探すのが後者です。CM 検出とも、お互いとも別の処理です。
...
進み具合は行に出ます（`黒白 検出中 33%`、`無音 検出中 60%`）。終わると
`黒白: 1 箇所`、`無音: 4 箇所` のように 1 行ずつ残ります。両方頼んだときは
...
上の「黒白を検出」（`Ctrl+B`）「無音を検出」（`Ctrl+Q`）で、いま開いている
録画を調べます。一覧で検出済みの録画なら、開いた時点で結果が並んでいます。
押すと読み直します。2 つは別々の処理なので、片方を押してももう片方の結果は
消えません。
