...
| 設定 | 内容 |
|---|---|
| **PageUp / PageDown** | そのキーの動きです。`Shift`、`Ctrl`、`Shift+Ctrl` の 4 通りに、それぞれ数と単位を持たせられます。単位は**フレーム移動**・**秒移動**・**% でスクロール**の 3 つです。前の 2 つは 1 回押すごとの移動量、% は移動速度で、押しているあいだ 1 秒に進む割合です。25 なら 4 秒で端から端まで進みます。割合はカットしたあとの長さに対するものなので、カットしても端から端までの時間は変わりません。既定は 15 フレーム、`Shift` で 30 フレーム、`Ctrl` で 25 %、`Shift+Ctrl` で 50 % です。0 にすると、そのキーは動きません |
