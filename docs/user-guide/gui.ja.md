...
黒白と無音の札は、検出した区間そのものから出します。区間の端をキーフレームに
したかどうかは関係ありません。区間の終わりの位置は、環境設定の
「黒白区間の終わりの印」に従います。

映像の左右に黒い余白があってインジケータが収まるときは、映像の上ではなく
余白に出します（4:3 の録画や、ウィンドウが映像より横に広いとき）。収まらない
ときは映像の角に重ねます。背景は半透明なので、下の局ロゴや時刻の表示も透けて
見えます。

要らないときは、最下段の**インジケータ**を押してください（既定はオン）。
...
- 字幕のない録画では、選択欄そのものが表示されず、最下段に並ぶのは「カウンタ」
  と「インジケータ」だけです。このページのほかのスクリーンショットがそうなっているのは、
  練習用の録画に字幕が無いためです。
