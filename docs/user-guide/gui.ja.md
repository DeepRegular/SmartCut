...
| 中央 | プレビュー。絵の下端中央にフレーム番号・時刻・そのコマの種類・選択範囲が出ます（最下段の**カウンタ**で消せます） |
...
| **ディスクの字幕** | ディスクから取り込んだ録画のときだけ出ます。既定は `カットの中に (PGS)` で、字幕の入った 1 ファイルになります。傍らに置くこともできます。`.idx / .sub` はプレイヤーも字幕ツールもそのまま読める 2 ファイル、`.sup` はディスプレイセットそのものです。**どれが無加工かは行に書いてあります**。DVD と Blu-ray とで別のものが無加工になるからです |
...
大きく出ている絵は、代表画像ではありません。**再エンコードされるコマそのもの**です。
