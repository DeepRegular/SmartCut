...
| **黒とみなす明るさ／白とみなす明るさ／その明るさが占める割合** | 何をもって「黒いフレーム」「白いフレーム」と呼ぶかです。画素の明るさと、そういう画素が画面に占める割合の 2 つで決めます。既定は黒 4 %、白 99 %、割合 98 % です。明るさは、放送の黒を 0 %、白を 100 % とした目盛りで表します。0 % にすると、黒そのものの画素だけを黒と数えます。8 ビットでも 10 ビットでも同じ値が使えます。フェードの終わりが真っ黒まで落ちない放送では、黒の値を 9〜11 % に上げると拾えるようになります。局のロゴや焼き込みの字幕、時計が出たままの録画では、割合を 95 % ほどまで下げてください。割合を上げると、黒と数えるフレームは減ります。ただし、区間の数が減るとはかぎりません。よくあるのは、黒地にクレジットが出る場面です。文字が出ているあいだは割合が下がるので、長い黒の区間がそこで途切れます。途切れた前後がどちらも長さの条件を満たしていれば、区間の数はかえって増えます。値は区間の数ではなく、見つかった区間の中身を見て決めてください。0.8.5 までは、明るさを 0〜255 全体に対する割合で表していました。放送の黒は 16 なので、6 % 以下では黒そのものが範囲に入らず、何も見つかりませんでした。以前の値は、新しい目盛りに換算して引き継ぎます。画面のいちばん外側 2 % は、どの値でも判定に使いません。放送の映像は端に半端な画素が並ぶことがあるためです。ここを変えると、それまでの検出結果は答えにならないので、各行の結果はいったん消え、検出し直すと録画を読み直します |
