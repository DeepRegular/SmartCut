...
書かれているあいだが 1 段階目です。左に並んでいるキーフレームは、録画の隣にあった
...
ある `.keyframe` や CM 検出結果のファイル、そして**ディスク**から開いた録画なら、
ディスク自身が打ったチャプターです。
...
| **キーフレーム情報を保存…** | 印の位置を録画の隣へ書き出す（`Ctrl+H`） |
...
| **CM 検出結果** | 検出した区間と、何で見つけたか。JSON | `録画.cm.json` |

保存画面で名前の拡張子を `.keyframe`、`.avs`、`.json` のどれかに書き換えれば、
そちらが優先されます。読み込みも同じで、選んだファイルの拡張子が形式を決めます。

`Ctrl+H` と `Ctrl+Shift+H` は、画面を出さずに録画の隣へそのまま保存します。
同じ名前のファイルがあれば確認します。確認せずに上書きする設定もあります。
CM 検出結果にショートカットはありません。

**CM 検出は録画を数分読みます。**結果を保存しておけば、次からはその数分を待たずに
同じ答えを開けます。検出そのものも一度走れば控えられるので、同じ録画を開き直した
ときは自動で戻ります。控えは SmartCut のキャッシュに入るので、別のパソコンへ
持っていくときや、検出結果を人に渡すときにファイルを使ってください。

カット編集を開いたときは、隣にあるこれらを自動で読み込みます。**keyframe は印が
付くだけです。Trim はカットされた状態で開きます。CM 検出結果は帯と印が付きます。**
いくつもあるときにどれを読むかは環境設定で決めます。1 つしか無いときは、設定に
関わらずそれを読みます。

**隣のファイルを読んだときは、CM 検出結果の印を混ぜません。**録画の隣に置いた
`.keyframe` は自分で出した答えで、CM 検出は機械が出した答えです。ひとつの列に
混ざると、あとからどちらの印か分かりません。検出結果はタイムラインの帯と下の行に
出ているので、印にしたければ **≡** の「CM 検出結果をキーフレームにする」を
押してください。

検出したときにいつも帯だけにしておくこともできます。環境設定の
**CM を検出したら、結果をキーフレームにする**（既定は入）です。
...
ここで出るのは書き出しのときです。カット編集の画面からも、録画の隣に同じものを
...
| **いくつもあるときに読むのは** | 録画の隣に `.keyframe`、`.trim.avs`、`.cm.json` がいくつもあるときに、どれを読み込むかです |
| **CM を検出したら、結果をキーフレームにする** | 既定は入です。外すと、検出しても帯と 1 行だけになり、印は ≡ の「CM 検出結果をキーフレームにする」で付けます。録画の隣のファイルを読んだときは、この設定に関わらず印を混ぜません |
...
| `Ctrl+H` | キーフレーム情報を録画の隣に保存 |
| `Ctrl+Shift+H` | Trim を録画の隣に保存 |
