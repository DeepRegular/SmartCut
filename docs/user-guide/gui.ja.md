...
中に入れるか傍らに置くかは[出力設定](#設定項目)で決めます。**メニュー**は
...
| `サムネイル` | 帯に並べる絵と、場面の変わり目を作っています（1 GB あたり 1〜2 秒くらい。4K の録画はもっとかかります） |
| `待機中` | まだ順番が来ていません |
...
**ほかのソフトも待たせません。** 下ごしらえは機械のコアを全部使いますが、
優先度を下げて動いています。ほかのソフトがコアを欲しがれば、そちらが先です。
機械が空いていれば下ごしらえの速さは変わりません。
...
| **サムネイルができたとき**（1 GB あたり 1〜2 秒。4K はもっとかかります） | 帯の絵がすべて埋まる、場面の変わり目、シークバーのホバー |
...
**左の列のキーフレームの絵も同じです。** 読み終わるまでは近いところで取った
絵が入り、読み終わると、印の時刻ちょうどの絵に入れ替わります。
...
| 中央 | プレビュー。絵の下端中央にフレーム番号・時刻・そのフレームの種類・選択範囲が出ます（最下段の**カウンタ**で消せます） |
| その下の帯 | **フィルムストリップ**。前後のフレームを絵で並べたものです。1 セルの長さは右の「表示」で選べます |
...
| 最下段 | 左にカットの費用、その隣に**カウンタ**と**字幕**（プレビューに何を出すか）。右に **OK** と**キャンセル** |
...
| `Space` または **▶ 再生** | ここから再生（映像と音声）。もう一度押すと停止。映像は録画そのもののフレームレートで出ます。間に合わない機械では、音に遅れる代わりにコマを間引いて出します |
...
最下段、カットの費用の隣に**字幕**があります。既定は「表示しない」で、選ぶとプレビューの
絵の上にその字幕が出ます。**カットの位置を字幕で決めるためのものです。** 継ぎ目が
台詞の途中に来ていないか、CM の前後で字幕がどこまで出ているかは、絵だけを見ていても
分かりません。
...
- 放送の**文字字幕（ARIB）**は、放送局が指定した位置・大きさ・色のまま文字として
  ...
  文字と同じ枠に描きます。
- ディスクの字幕（**PGS** と DVD の**サブピクチャ**）は絵なので、ディスクが描いた
  ものをそのまま重ねます。
- 字幕が 2 本以上あるとき（二か国語、日本語と英語など）は、選んだ 1 本だけを出します。
- フレーム番号と時刻の表示は、字幕を出していても絵の**下**のままです。字幕と同じ場所
...
| **トランスコードする** | 一覧が選んだディスクに収まらないとき、映像をトランスコードして収めます。**収まるときは何もしません。** 音声・字幕・番組情報はそのままで、小さくなるのは映像だけです。オンにすると、下の図に「変換後」の行が増えて、映像をどのくらいのサイズにするかが出ます。MPEG-2 の録画だけが対象で、それ以外の録画は元のサイズのまま場所を取ります。中で何をしているかは[ディスクに収める](../technical/transrate.ja.md)へ |
| **ディスクの使用量** | 一覧がディスクに対してどれだけかを、録画 1 本ずつの帯にして描きます。白い線がディスクの端で、その手前の細い線が残しておく 1% です。線を越えた部分は赤くなります。数字は書き出す前の見積もりで、丸ごと 1 本なら 1% ほどの精度です。単位は GiB で、ファイルマネージャーが言うのと同じ 1024 換算です。1 層のディスクが 23.3 GiB と出るのはそのためで、上の「25GB」は箱に書いてある名前のほうです |
...
一覧の控えが、キュー専用の置き場に書かれます。** 出力先には出しません。名前は、
開いているプロジェクトがあればその名前、無ければ一覧の先頭の行の名前です。控えは
キューの持ち物で、バッチ出力ツールでそのジョブを消すと一緒に消えます。

**登録は保存でもあります。** 開いているプロジェクトがあれば、そのファイルにも
書きます。無題の一覧なら、控えが書かれたことをもって保存済みとして扱い、
タイトルバーの `*` が消えます。登録すると
...
大きく出ている絵は、代表画像ではありません。**再エンコードされるフレームそのもの**です。
...
この項目はここにしかありません。プロジェクトには保存されない、常設の答えです。
...
| 設定 | 内容 |
|---|---|
| **範囲の先頭を整え直す** | 開いた GOP から始まる範囲で、先頭の最大 2 秒を再エンコードします。継ぎ目の乱れは減りますが、そのぶん無劣化でコピーされる区間が短くなります。CLI の `--clean-joins` と同じ設定です |
...
このプログラムに割り当てるキャッシュの場所を使います。**参照…** で変更できます。
...
このプログラム自身が表示するためです。不具合を報告するときにだけ使ってください。
...
| **編集画面の絵が粗い / 出るのが遅い** | まだ読み込みの途中です。読み込みが終わればフレーム単位で正確になり、サムネイルができれば帯が残らず埋まります（[開いた瞬間から使えます](#開いた瞬間から使えます)）。索引は初回だけ作ります |
