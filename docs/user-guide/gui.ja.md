...
| その下の帯 | **フィルムストリップ**。前後のフレームをサムネイルで並べたものです。1 画面に映る秒数は右の「1 画面」で選べます。セルの幅は、そのセルが表す時間に比例します。いまの位置は中央の縦線で示します。「1 フレームずつ」のときは 1 セルが 1 フレームなので、縦線の代わりに、表示中のフレームを青い枠で囲みます |
