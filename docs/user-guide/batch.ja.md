...
| **サムネイル** | 帯に並べる絵と、場面の変わり目を作る | 1 GB あたり 1〜2 秒くらい。4K の録画はもっとかかります |
...
一覧 1 つは、一晩ぶんの仕事です。**バッチ出力ツール**が扱うのはその外側です。
...
**そのファイルは、キューが持つ控えです。** `バッチに登録` でも `ジョブ追加` でも、
キューは自分の置き場に控えを書いて、それを実行します。並べたあとに元の
プロジェクトを直してもキューの中身は変わりませんし、行を消しても元のファイルは
残ります。控えを直したいときは行の `プロジェクトを開く` からどうぞ。
...
立ち上がります。** すでに走っている最中でも構いません。追加ぶんは処理中のジョブの
...
**左の絵は、いま作っているフレーム**です。順番が来るまでは 1 本目の録画の絵を出して
います。書き出しが始まると、出力画面のプレビューと同じ絵に変わります。映るのは
再エンコードされるフレームだけです。つまり、つなぎ目でこのプログラムが作っている
絵が出ます。出力画面を開いていなくても、キューの画面だけで同じものが見えます。
...
プロジェクトファイルの名前はどこにも出ません。キューが走らせているのは自分の置き場に
作った控えなので、そのファイル名は仕事の中身について何も言わないからです。読めない
...
選んだ行が複数あれば、まとめて運びます。掴んで運ぶあいだ、どの隙間に落ちるかが
線で出ます。Escape で元に戻ります。
...
| `ジョブ削除` | 選んだジョブをキューから外します。キューが持っていた控えも一緒に消えます。元のプロジェクトはそのままです |
