...
一覧 1 つは、一晩ぶんの仕事です。4 つ目のタブ `バッチ出力` はもう 1 本の軸です。
...
②  バッチ出力 → この一覧を追加
③  次の晩のぶんも同じようにする
④  バッチ出力ツール を押し、開いた窓で バッチ開始 を押して寝る
```

同じことは `出力` 画面からもできます。`出力開始` の下の `バッチに登録` です。
出力設定を決めたあとなら、「いま書く」か「キューに回すか」は同じ問いの 2 つの
答えだからです。どちらの経路でも、一覧はまずプロジェクトとして保存されます
（名前が無ければそこで訊かれます）。

`プロジェクトを追加…` では、以前保存したプロジェクトをまとめて選べます。
`上へ` `下へ` で順番を変え、`削除` で 1 件外し、`全消去` で空にします。

**キューはプログラムを閉じても残ります。** 変更のたびにファイルへ書き出して
いるので、深夜に並べたキューは朝もそのままです。書き終わったジョブは結果を
つけたまま一覧に残り、二度は書きません。不要になったら `削除` で外してください。

### ツールは別ウィンドウで動く

`バッチ出力ツール` を押すと、**別ウィンドウが、別プロセスとして**立ち上がります。
そこが肝心なところです。メインのウィンドウを閉じても、SmartCut を終了しても、
書き出し中のキューは止まりません。ツールにはキューと `出力` 画面しかなく、
`バッチ開始` もそちら側にあります。

2 つのウィンドウは同じキューを数秒おきに読みます。ですからメイン側でも、ツールが
どのジョブまで進んだかが見えます。**ツールの実行中でもプロジェクトは追加できます。**
追加ぶんはキューの末尾に入り、ツールはいま処理中のジョブを終えたところで拾います。
順番の入れ替えと削除は、ツールが終わるまで待ってください。実行中のキューの行は
ツールのものです。
