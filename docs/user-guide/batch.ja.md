...
**動かすボタンは 1 つ**で、ツールのバーの左端にあります。待機中は `バッチ開始`、
書き出し中は赤い `バッチ中止` になります。ツールにタブはありません。待っている
あいだはキュー、動いているあいだは書き出しの様子を出し、切り替えは自分で行うので、
どちらが出ていてもそのボタンに手が届きます。その隣に、キューの中身を作る 2 つが
並びます。`ジョブ追加…` は以前保存したプロジェクトをまとめて選べます。`ジョブ削除`
はクリックで選んだ行を外します。`ジョブ削除` の右端の `▾` には、まとめて外す
2 つ、`出力済のジョブを削除` と `すべて削除` が入っています。**ツールが開いていてもいなくても**
...
`バッチ中止` を押すと、実行中のジョブもキューも止まります。いま書いている録画は
最後まで書き終えてから止まるので中途半端なファイルは残らず、届かなかったジョブは
待機に戻ります。
