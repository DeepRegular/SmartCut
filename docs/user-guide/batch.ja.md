...
いま画面にある一覧をキューの末尾に入れます。**ツールが開いていてもいなくても**
...
専用フォルダーがあればそれも、ディスクなら `/BDAV` まで。入力の隣に書く設定の
プロジェクトは、そう書いてあります。1 つのフォルダーとして名指しできないからです。
その下の行がどのプロジェクトで録画を何本持っているか、さらに下がバーです。
