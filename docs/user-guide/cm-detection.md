...
Measured against a ground truth built by eye, for five real recordings:
...
- **The silence is long.** About 1 second at a seam, against 0.1–0.4 s for a pause
  in the programme.
- **It lands on a 15-second grid.** The neighbouring seam is exactly 15, 30 or 60
  seconds away.
...
|---|---|---|
| **Subtitle resets** | A mark the broadcaster's own equipment stamps into the caption stream at each seam | 3 seconds |
...
The detector does not use that property at all, which makes it independent evidence
that the boundaries landed on real cuts. If a block comes out at 119.8 seconds
rather than 120.0, something is slightly off.

## What to do when it gets one wrong

- **A block is a little short or long.** Open the cut editor, move IN or OUT to the
  keyframe you want, and cut from there. The marks are a starting point, not a
  verdict.
- **A break was missed.** `S` and `Shift+S` step through scene changes, which is
  usually the fastest way to find a boundary by hand.
- **A block covers programme material.** Delete the marks you do not want (the `×`
  on the card) and cut around it. This is the expensive error, and the detector is
  tuned to avoid it, but it can still happen on material with unusual pacing.
- **Nothing was found at all.** Some recordings genuinely have no commercials.
  Otherwise, see "When the fallback is not good enough" below.
...
The seam sits in the middle of about a second of silence, so moving it a few hundred
milliseconds is inaudible. That small concession is what makes the whole commercial
cut lossless.
...
The rest of this page is the implementation and the measurements behind it
(`cm.rs`, `logo.rs`, `caption.rs`).
